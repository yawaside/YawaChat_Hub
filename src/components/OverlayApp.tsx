...
import { fmtAmount } from "../lib/donation";
...
export function OverlayOnlineBar({
  items, look, fontSize, mode, showBorder, radius, donationCurrency = "RUB",
}: {
  ...
  showBorder: boolean;
  radius: number;
  /** Валюта донатов — для суммы DonationAlerts в строке онлайна. */
  donationCurrency?: string;
}) {
  if (!items.length) return null;
  // Даже если обновления коннекторов пришли в другом порядке, DonationAlerts
  // всегда закреплён первой площадкой в оверлее.
  const orderedItems = [...items].sort((a, b) => {
    if (a.platform === "donation") return -1;
    if (b.platform === "donation") return 1;
    return 0;
  });
  ...
  return (
    ...
    >
      {orderedItems.map((p) => {
        ...
        /* DonationAlerts — площадка с общей суммой донатов за стрим,
           а не числом зрителей: показываем сумму в валюте канала. */
        if (p.platform === "donation") {
          const total = p.viewers ?? 0;
          return (
            <span
              key={p.platform}
              className="inline-flex items-center gap-1.5 leading-none"
              title={`${meta.label}: собрано за стрим ${fmtAmount(total, donationCurrency)}`}
            >
              ...
              <span className="font-mono tabular-nums" style={{ color: meta.color }}>
                {fmtAmount(total, donationCurrency)}
              </span>
            </span>
          );
        }
...
export default function OverlayApp() {
  ...
  /* Валюта донатов — одна настройка на приложение и оверлей. */
  ...
  // Те же смайлы, что и в основной ленте: глобальные наборы + BTTV/7TV/FFZ/Kick
  // конкретных подключённых каналов (раньше оверлей грузил только глобальные).
  useEmotes(channels);
  ...
  useEffect(() => {
    ...
    const off = sp.overlay.onChange((o) => setCfg((cur) => ({ ...cur, ...DEFAULT_OVERLAY, ...o })));
