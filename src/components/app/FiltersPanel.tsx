...
const SCOPES: Scope[] = ["all", ...PLATFORM_IDS.filter((p) => p !== "donation")];
const SCOPE_NAMES: Record<Scope, string> = { all: "Общие", twitch: "Twitch", youtube: "YouTube", kick: "Kick", tiktok: "TikTok", vk: "VK Live", donation: "DonationAlerts" };
  ...
  return <div className="settings-page" data-settings-page="filters">
    ...
    <div id={`${uid}-content`} role="tabpanel" aria-labelledby={`${uid}-${section}`}>
      ...
      {section === "test" && <Panel title="Проверить сообщение" desc="Проверка учитывает выбранную площадку, активные пресеты и личные правила.">
        <SettingRow label="Площадка"><Select label="Площадка проверки" value={testPlatform} onChange={setTestPlatform} options={PLATFORM_IDS.filter((id) => id !== "donation").map(id => ({ id, label: SCOPE_NAMES[id] }))} /></SettingRow>
