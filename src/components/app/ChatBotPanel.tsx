...
import { Bot, CheckCircle2, Command, ExternalLink, FlaskConical, LogOut, Plus, Send, Trash2, Zap } from "lucide-react";
 ...
 * скачивает приложение, нажимает «Авторизоваться» на Twitch/VK в браузере по
 * умолчанию — канал и профиль подтягиваются автоматически. Ни логин, ни токен
 * вручную вводить не требуется.
 */
      ...
      <div className="rounded-2xl border p-3.5" style={{ borderColor: connected ? `${meta.color}55` : "var(--dw-line)", background: connected ? `color-mix(in srgb, ${meta.color} 7%, var(--dw-panel2))` : "var(--dw-panel2)" }}>
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-[10.5px] leading-snug" style={{ color: "var(--dw-dim)" }}>
              {connected
                ...
                : "Вход откроется в браузере по умолчанию; канал определится автоматически."}
            </p>
          </div>
        </div>
        <div className="mt-3 flex gap-2">
          {connected ? (
            ...
            <Btn variant="primary" disabled={pending || !sp?.oauth} onClick={() => authorize(id)} className="h-8 flex-1"><ExternalLink size={12} /> {pending ? "Открываю браузер…" : "Авторизоваться"}</Btn>
          )}
        </div>
      </div>
    );
