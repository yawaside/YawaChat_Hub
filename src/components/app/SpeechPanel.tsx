import { ListX, Mic, Pause, Play, SkipForward, VolumeX } from "lucide-react";
import type { SpeechEngine } from "../../lib/core";
import { Eq } from "../bits";
import { Btn, Label, Segmented, Toggle } from "./ui";
    ...
    >
      ...
      <div className="mb-2 flex items-center justify-between px-0.5">
        ...
        <span
          ...
          style={{
            background: speech.enabled ? "#4ade80" : "var(--dw-line)",
            boxShadow: speech.enabled ? "0 0 8px rgba(74,222,128,0.6)" : "none",
          }}
        />
      </div>
      <div className="flex flex-wrap items-center gap-1.5">
        ...
        >
          {speech.enabled ? (
            <Eq active={!speech.paused && !!speech.now} color="#fff" size={12} />
          ) : (
            <VolumeX size={12} />
          )}
          {speech.enabled ? "Озвучка вкл" : "Озвучка выкл"}
        </Btn>
        <Btn
          variant="ghost"
      ...
      <div className="mt-2 flex min-h-[16px] items-center gap-2">
        <Eq
          ...
          color={speech.enabled ? "var(--dw-accent)" : "var(--dw-line)"}
          size={13}
        />
        <span
