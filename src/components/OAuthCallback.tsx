...
export default function OAuthCallback({ hash }: { hash: string }) {
  ...
  useEffect(() => {
    ...
    const state = params.get("state") || fromQuery.get("state") || "";
    const p: OAuthPlatform = state.startsWith("twitch") || fromQuery.get("platform") === "twitch"
      ? "twitch"
      : state.startsWith("vk") || fromQuery.get("platform") === "vk"
        ? "vk"
        : "twitch";
