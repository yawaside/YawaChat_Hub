 ...
 *
 * Twitch и VK возвращают пользователя на этот сайт по-разному:
 *   — Twitch (implicit flow): #access_token=…&state=… (фрагмент адреса);
 *   — VK (OAuth 2.1 + PKCE):  ?code=…&state=…&device_id=… (query-параметры).
 *
 * Здесь мы:
 *   1. читаем данные из фрагмента/query;
 *   2. определяем площадку по префиксу state: "twitch_" или "vk_";
 *   3. НЕ показываем токен/код пользователю;
 *   4. передаём данные в локальный сервер приложения (127.0.0.1:17563) —
 *      для VK код обменивается на токен УЖЕ в приложении (у сайта нет
 *      code_verifier и он не может это сделать сам);
 *   5. приложение само сверяет state и возвращает фокус в окно.
 *
 * Ничего секретное никогда не отображается на экране и не уходит никуда,
 * кроме запущенного приложения на этом компьютере.
 */
...
function parseCallbackUrl(hash: string, search: string) {
  ...
  // VK ID может возвращать JSON-строку payload={code,state,device_id}
  let payloadObj: Record<string, string> = {};
  const payloadRaw = fromQuery.get("payload") || fromHash.get("payload");
  if (payloadRaw) {
    try { payloadObj = JSON.parse(payloadRaw); } catch { /* noop */ }
  }

  const get = (k: string) => payloadObj[k] || fromHash.get(k) || fromQuery.get(k) || "";
  ...
  // Площадка — по префиксу state (app шлёт «twitch_<rand>» или «vk_<rand>»)
  // либо по наличию code/device_id (VK ID) vs access_token (Twitch).
  let platform: OAuthPlatform | null = null;
  if (state.startsWith("twitch_")) platform = "twitch";
  else if (state.startsWith("vk_")) platform = "vk";
  else if (token && !code) platform = "twitch";
  else if (code || deviceId) platform = "vk";

  return { token, code, deviceId, state, error, platform };
}
...
export default function OAuthCallback({ hash }: { hash: string }) {
  ...
  /**
   * Отправляет данные авторизации в локальный сервер приложения.
   * Для Twitch — готовый access_token; для VK — code+device_id: обмен на
   * токен выполняет само приложение (у сайта нет code_verifier для PKCE).
   */
  const deliver = (payload: Record<string, string>, target: OAuthPlatform) => {
    ...
    send()
      .then(() => {
        setPhase("ok");
        // Автоматически возвращаем фокус в приложение
        try { window.location.href = "yawachat://oauth"; } catch { /* noop */ }
      })
      .catch(() => {
        // Запасной канал: GET с параметрами
        const qUrl = `${BRIDGE_HOST}/auth/${target}?` + new URLSearchParams(payload).toString();
        fetch(qUrl, { mode: "no-cors", cache: "no-store" })
          .then(() => {
            setPhase("ok");
            try { window.location.href = "yawachat://oauth"; } catch { /* noop */ }
          })
  ...
  useEffect(() => {
    ...
    const { token, code, deviceId, state, error, platform: p } = parseCallbackUrl(hash, search);
    if (error) {
      setPhase("error");
    ...
    if (p === "twitch") {
      ...
      deliver({ token, state }, p);
    } else {
      if (!code) { setPhase("failed"); return; }
      deliver({ code, device_id: deviceId, state }, p);
    }
