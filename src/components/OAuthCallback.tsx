...
function parseCallbackUrl(hash: string, search: string) {
  ...
  return { token, state, error, platform };
}
...
export default function OAuthCallback({ hash }: { hash: string }) {
  ...
  const deliver = (token: string, state: string, target: OAuthPlatform) => {
  ...
  useEffect(() => {
    ...
    const { token, state, error, platform: p } = parseCallbackUrl(hash, search);
    if (error) {
      setPhase("error");
