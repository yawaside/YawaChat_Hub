...
export default function OAuthCallback({ hash }: { hash: string }) {
  ...
  const deliver = (payload: Record<string, string>, target: OAuthPlatform) => {
    ...
    const endpoint = `${BRIDGE_HOST}/auth/${target}?${qs}`;

    // 1) POST fetch с no-cors
    const sendPost = () =>
      fetch(endpoint, {
        method: "POST",
        mode: "no-cors",
    ...
    // 2) Image GET beacon: не блокируется CORS/PNA браузера
    const sendGetBeacon = () => {
      try {
        const img = new Image();
        img.src = endpoint;
    ...
    // Автовозврат в приложение: через 600 мс пробуем активировать окно через yawachat://
    window.setTimeout(() => {
      try {
    ...
    // Подстраховка: повтор через 1.5 с, если приложение просыпалось
    window.setTimeout(() => {
      if (phase !== "ok") {
        sendPost().catch(() => {});
        sendGetBeacon();
