...
const MAX_FEED = 300;
/* Сообщения из главного процесса приходят пачками (несколько площадок сразу).
   Отдельный setState на каждое сообщение перерисовывал всю ленту десятки раз
   в секунду — отсюда рывки прокрутки. Копим их и применяем одним обновлением
   не чаще раза в кадр анимации. */
const FEED_FLUSH_MS = 90;
...
export default function DesktopApp() {
  ...
  useEffect(() => {
    ...
    const off = sp.overlay.onChange((o) => setOverlayState({ ...DEFAULT_OVERLAY, ...o }));
  ...
  const pendingFeed = useRef<ChatMsg[]>([]);
  const flushTimer = useRef<number | null>(null);

  const flushFeed = useCallback(() => {
    flushTimer.current = null;
    const batch = pendingFeed.current;
  ...
  const push = useCallback((m: ChatMsg) => {
    ...
    // Озвучка идёт сразу: очередь TTS не должна ждать отрисовку.
    speechRef.current.enqueue(m);
    pendingFeed.current.push(m);
    // Защита от всплеска: не копим больше, чем помещается в ленту.
    if (pendingFeed.current.length > MAX_FEED) {
      pendingFeed.current = pendingFeed.current.slice(-MAX_FEED);
    }
    if (flushTimer.current === null) {
      flushTimer.current = window.setTimeout(flushFeed, FEED_FLUSH_MS);
    }
  }, [flushFeed]);
