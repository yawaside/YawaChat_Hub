    ...
    >
      ...
      >
          ...
          <div className="min-w-0">
            {variant.id === "terminal" ? (
              <div className="truncate font-mono text-[13px] font-bold leading-none tracking-[0.08em]" style={{ color: "var(--dw-text)" }}>
                YAWACHAT_HUB
                <span className="ml-0.5 animate-breathe" style={{ color: "var(--dw-accent)" }}>▌</span>
              </div>
            ) : (
        ...
        <button
          ...
          className="ml-auto hidden items-center gap-2 rounded-full border px-3.5 py-2 text-[11px] font-bold transition-all hover:border-[var(--dw-accent)] sm:inline-flex"
          style={{
            borderColor: ttsSafe.enabled ? "rgba(34,211,238,0.45)" : "var(--dw-line)",
            background: ttsSafe.enabled ? "rgba(34,211,238,0.09)" : "var(--dw-input)",
            color: ttsSafe.enabled ? "#67e8f9" : "var(--dw-dim)",
          }}
        >
          {/* Полосы анимируются, когда озвучка работает с очередью или говорит. */}
          <Eq
            color={ttsSafe.enabled ? "#22d3ee" : "var(--dw-dim)"}
            active={speech.enabled && !speech.paused && (!!speech.now || speech.queueSize > 0)}
          />
          <span className={ttsSafe.enabled ? "" : "opacity-70"}>
            {ttsSafe.enabled ? (speech.paused ? "пауза" : "озвучка вкл") : "озвучка выкл"}
          </span>
        </button>
