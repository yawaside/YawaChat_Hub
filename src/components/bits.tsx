...
export function PlatformIcon({ id, size = 16, color, className, style }: {
  ...
  const Icon = { twitch: TwitchIcon, youtube: YoutubeIcon, kick: KickIcon, tiktok: TiktokIcon, vk: VkIcon }[id];
  ...
  return (
    <span data-platform-badge={id} title={meta.label} aria-label={meta.label}
      style={{ display: "inline-grid", placeItems: "center", width: px, height: px,
        ...
        background: plain ? "transparent" : meta.color,
...
/* ---------- equalizer bars ----------
   Полосы движутся только когда active === true (реальная речь или демо).
   В покое показываем статичные столбики — иначе индикатор прыгает бесконечно. */
export function Eq({ color = "#8b5cf6", active = true }: { color?: string; active?: boolean }) {
  // фиксированные высоты для статичного вида: разной формы, но без движения
  const idleHeights = [0.5, 0.3, 0.42, 0.24];
  return (
    <span className="flex h-3.5 items-end gap-[2.5px]" aria-hidden="true">
      {(active ? [0.9, 0.5, 0.7, 0.35] : idleHeights).map((d, i) => (
        <span
          key={i}
          className={`w-[3px] origin-bottom rounded-full ${active ? "animate-eq" : ""}`}
          style={
            active
              ? { height: "100%", background: color, animationDelay: `${i * 0.14}s`, animationDuration: `${d + 0.25}s` }
              : { height: `${idleHeights[i] * 100}%`, background: color, transform: "scaleY(1)" }
          }
        />
      ))}
    </span>
