...
export default function App() {
  ...
  /* Возврат с площадки авторизации: в адресе есть access_token (Twitch)
     или authorization code (VK ID OAuth 2.1). Показываем страницу-мост,
     которая передаёт данные в запущенное приложение без показа секретов. */
  const isOAuthReturn =
    typeof window !== "undefined" &&
    (/access_token=/.test(window.location.hash + window.location.search) ||
     (/code=/.test(window.location.search) && /state=/.test(window.location.search))) &&
