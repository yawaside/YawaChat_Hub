...
export function aggregateViewers(channels: Channel[]): PlatformOnline[] {
  ...
  for (const platform of PLATFORM_IDS) {
    // DonationAlerts закреплён первой площадкой в оверлее всегда: до настройки
    // показывает нулевую сумму, после подключения — итог донатов за стрим.
    // Остальные площадки попадают в строку только при реальном online-статусе.
    const online = channels.filter((c) => c.platform === platform && (platform === "donation" || c.status === "online"));
...
export function totalViewers(list: PlatformOnline[]): number | null {
  // Сумма DonationAlerts — не число зрителей и не участвует в общем онлайне.
  const known = list.filter((p) => p.platform !== "donation" && p.viewers !== null);
