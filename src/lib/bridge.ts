...
export function useBotAuth(toast: (t: string) => void) {
  ...
  useEffect(() => {
    refresh();
    if (!sp?.oauth) return;
    const off = sp.oauth.onToken(() => {
      refresh();
      toast("Бот подключён — ник канала получен автоматически");
    });
    return off;
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [sp]);
  ...
  const authorize = useCallback(
    async (platform: BotPlatform) => {
      ...
      setBusy("");
      if (!res.ok) toast(res.error || "Не удалось открыть браузер");
      else toast("Браузер открыт — разрешите доступ и вернитесь в приложение");
    },
    [sp, toast]
  );
