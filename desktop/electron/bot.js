...
class BotManager {
      ...
      for (const line of raw.toString().split("\r\n")) {
        ...
        if (msg.command === "001" || msg.command === "JOIN" || msg.command === "366") {
          if (b.status !== "online") {
            ...
            // Награды за баллы канала (channel points) видны ТОЛЬКО через EventSub —
            // раньше подписки на этот топик не было вовсе, поэтому события не показывались.
  ...
  async handleTwitchEventSubMessage(b, cfg, ws, raw) {
    ...
    if (subType === "channel.channel_points_custom_reward_redemption.add") {
      ...
      this.emit("twitch", { author: who, text: `${who} активировал(а) награду «${reward}»${input}`, badges: ["BOT"], kind: "sys.reward" });
    }
  }

  /** Оформляет подписку на EventSub-топик через Helix (нужен broadcaster/moderator токен). */
  async subscribeTwitchEventSub(cfg, sessionId) {
    try {
      ...
      // 409 — подписка уже существует (например, после reconnect) — это не ошибка.
