...
const { getJson, request, tlsOptionsFor } = require("./net");
...
class BotManager {
  ...
  connectVk(cfg) {
    ...
    /* Формат авторизации из документации vklive-message-client:
     *   auth: { accessToken: <токен live.vkvideo.ru> } — чтение И отправка (владелец)
     *   auth: "readonly"                                  — только чтение (анонимно)
     *
     * Порядок и надёжность:
     *   — если есть токен — сначала пробуем «под владельцем» (полный доступ);
     *   — если токен отклонён или его нет — анонимно readonly: чтение работает без него;
     *   — ОБА клиента получают ОДИН и тот же обработчик (wireVk), поэтому даже
     *     переключение на readonly не теряет ни одного сообщения.
     */
    const makeClient = (auth) => {
      try {
        return new VKPLMessageClient({ auth, channels: [channel], debugLog: false, log: false });
      } catch (e) {
        console.warn("[vk] не удалось создать клиент:", e && e.message);
        return null;
      }
    };
    ...
    const wireVk = (cl) => {
      cl.on("message", (ctx) => {
        ...
        if (!text) return;
        const author = (ctx && ctx.user && (ctx.user.nick || ctx.user.displayName || ctx.user.name)) || "зритель";
        const badges = [];
        if (ctx && ((ctx.message && ctx.message.author && (ctx.message.author.isAdmin || ctx.message.author.isOwner)) || (ctx.user && (ctx.user.isAdmin || ctx.user.isOwner)))) badges.push("MOD");
      ...
      try {
        cl.on("error", (e) => {
          this.emitEvent("vk", "bot.error", `VK: ${String(e && e.message || e).slice(0, 100)}`);
          if (b.alive && b.status === "online") this.setStatus(b, "error", String(e && e.message || e).slice(0, 90));
        });
      } catch { /* noop */ }
    };
    ...
    (async () => {
      try {
        let cl = null;
        let mode = "readonly";
        // 1) «Под владельцем» — полный доступ (чтение + отправка + модерация).
        const tokAuth = { accessToken: token };
        if (token) {
          const withToken = makeClient(tokAuth);
          if (withToken) {
            try {
              wireVk(withToken);
              ...
              console.warn("[vk] подключение с токеном не удалось:", msg);
              this.emitEvent("vk", "bot.error", `VK: токен не принят — продолжаю в режиме только чтение (${msg.slice(0, 60)})`);
              try { withToken.disconnect && withToken.disconnect(); } catch { /* noop */ }
            }
          }
        }
        // 2) Анонимно readonly — чтение работает без токена.
        if (!cl) {
          const ro = makeClient("readonly");
          if (ro) {
            try {
              wireVk(ro);
              await ro.connect();
              ...
              this.setStatus(b, "error", `VK: подключение не удалось — ${String(e && e.message || e).slice(0, 80)}`);
              return;
            }
          } else {
            this.setStatus(b, "error", "VK: не удалось создать клиент чата");
            return;
          }
        }
        if (!b.alive) return;
        ...
        this.setStatus(b, "online");
        if (firstJoin) {
          this.emitEvent("vk", "bot.join", `VK: чат-бот подключён к ${channel} (${mode === "token" ? "полный доступ · владелец" : "только чтение"})`);
          if (mode === "token") this.watchVkFollowers(b, token, channel);
        }
      } catch (e) {
        if (!b.alive) return;
        this.setStatus(b, "error", String(e && e.message || e).slice(0, 90));
      }
    })();
  }
  ...
  sendChat(platform, channel, text) {
    ...
    if (platform === "vk" && b && b.status === "online" && b.vkClient) {
      try {
        // Документация: sendMessage(text, channel) — СНАЧАЛА текст, ПОТОМ канал.
        // Раньше аргументы стояли наоборот, поэтому сообщения уходили «в никуда».
        if (typeof b.vkClient.sendMessage === "function") {
          b.vkClient.sendMessage(body.slice(0, 480), target);
          return true;
        }
        if (typeof b.vkClient.send === "function") { b.vkClient.send(body.slice(0, 480), target); return true; }
      } catch (e) {
        this.emitEvent(platform, "mod.error", `VK: ошибка отправки — ${String(e && e.message || e).slice(0, 80)}`);
        return false;
      }
    }
  ...
  async moderate(platform, { author, login: userLogin, action, seconds, text }) {
    ...
    if (platform === "twitch") {
      ...
      /* broadcaster_id и moderator_id.
         Если авторизованный аккаунт — владелец канала (обычный случай), они равны.
         Если бот — модератор чужого канала, broadcaster_id находим по нику канала. */
      let broadcasterId = cfg.accountId;
      let moderatorId = cfg.accountId;
      const chan = login(cfg.channel);
      if (chan && chan !== login(cfg.account)) {
        broadcasterId = await this._twitchUserIdById(clientId, token, chan).catch(() => cfg.accountId);
      }

      /* Helix возвращает {ok, status, body}; РАНЬШЕ ответ не проверялся — при
         401/403 (нет scopes, неверный токен, нет прав модератора) код молча
         «сообщал об успехе», и модерация не происходила. Теперь проверяем и
         показываем реальную причину. */
      const helixJson = async (path, method, body) => {
        const res = await this.helix(path, token, method, body);
        if (res && res.ok) return { ok: true, res };
        let detail = "";
        try {
          const j = JSON.parse(res && res.body || "{}");
          detail = (j.message || j.error || j.error_description) || "";
        } catch { /* noop */ }
        return { ok: false, error: `Twitch API ${(res && res.status) || 0}${detail ? ": " + detail : ""}`.trim(), res };
      };
      ...
      if (action === "emoteonly" || action === "subscribers" || action === "clear") {
        try {
          const key = action === "subscribers" ? "subs" : action;
          const cfgBody = action === "subscribers" ? { ...MODES[key].data, subscriber_mode: true } : MODES[key].data;
          const r = await helixJson(MODES[key].path, "PATCH", cfgBody);
      ...
      if (action === "slow") {
        try {
          ...
          const r = await helixJson("/chat/settings", "PATCH", {
      ...
      try {
        ...
        if (action === "ban") {
          const r = await helixJson(path, "POST", { data: { user_id: userId, reason: "YawaChatHub" } });
          if (!r.ok) return { ok: false, error: r.error };
          this.emitEvent("twitch", "mod.ban", `${target} забанен`);
        } else if (action === "unban") {
          const r = await helixJson(path, "DELETE", { data: { user_id: userId } });
          if (!r.ok) return { ok: false, error: r.error };
          this.emitEvent("twitch", "mod.unban", `${target} разбанен`);
        } else if (action === "timeout") {
          const secs = Math.max(1, Number(seconds) || 300);
          const r = await helixJson(path, "POST", { data: { user_id: userId, duration: secs, reason: "YawaChatHub" } });
          if (!r.ok) return { ok: false, error: r.error };
    ...
    if (platform === "vk") {
      ...
      if (b.vkAuthMode !== "token") {
        return { ok: false, error: "VK подключён только на чтение — нужен токен для модерации" };
      }

      /* ВАЖНО: postJson из net.js ВОЗВРАЩАЕТ null при ошибке и не бросает
         исключений. Раньше здесь стоял .catch(), который никогда не срабатывал,
         а метод сообщал об успехе даже когда запрос провалился. Теперь
         результат проверяется явно. */
      const vkApi = (endpoint, payload) => {
        const hosts = ["https://api.live.vkvideo.ru", "https://api.vkplay.live"];
        const body = JSON.stringify(payload || {});
        // Перебираем оба домена: живой — live.vkvideo.ru, старый — vkplay.live.
        return hosts.reduce(
          (chain, host) => chain.then((res) => (res ? res : request(`${host}/v1/blog/${encodeURIComponent(cfg.channel)}/${endpoint}`, {
            method: "POST",
            headers: { "content-type": "application/json", Authorization: `Bearer ${cfg.token}`, accept: "application/json" },
            body,
            timeout: 12000,
          }).then((r) => (r && r.ok ? r.body : null)).catch(() => null))),
      ...
      try {
        ...
        if (action === "unban") {
          // VK Live: снятие бана. Пробуем оба известных эндпоинта.
          let res = await vkApi("unban", { user: target });
          if (!res) res = await vkApi("remove_ban", { user: target });
          if (!res) return { ok: false, error: `VK не снял бан с ${target} (эндпоинт недоступен)` };
          this.emitEvent("vk", "mod.unban", `${target} разбанен`);
          return { ok: true };
        }
        // ban: удаление пользователя из чата.
        const banned = await vkApi("remove_user", { user: target });
        if (!banned) return { ok: false, error: `VK не забанил ${target} — проверьте права модератора` };
