...
class BotManager {
  constructor({ settings, onChat, onBotStatus, onChannels, onToken, onAuthorizedChannel }) {
    this.settings = settings;
    this.onChat = onChat;
    this.onBotStatus = onBotStatus;
    this.onChannels = onChannels;
    this.onToken = onToken;
    // Вызывается сразу после успешной авторизации с известным каналом —
    // main.js подключает его через обычный ConnectorManager (виден в «Каналы»,
    // есть онлайн/зрители), а дубли текста сообщений гасятся в connectors.js
    // (см. _isAuthManaged): текст чата идёт только от авторизованного бота.
    ...
    this.recentRewards = new Set(); // дедупликация наград EventSub / IRC
  }

  /* ---------------- сообщения (автор бота строго YawaChatHub) ---------------- */
  ...
  emit(platform, { author, text, color, badges, kind, amount, currency }) {
    ...
    this.onChat({
      ...
      platform,
      // Название бота, который пишет события и прочие действия — строго YawaChatHub
      author: author || "YawaChatHub",
      text: String(text),
      color: color || "#4c8dff",
      badges: badges || [],
      ts: Date.now(),
  ...
  /* ============================================================
     Авторизация: токен получен (Twitch — implicit, VK — PKCE code exchange)
     ============================================================ */
  ...
  async handleOAuthToken(platform, token, extra) {
    ...
    if (b) b.joinAnnounced = false; // сбрасываем, чтобы соединение выдало ровно одно уведомление
  ...
  async resolveAccount(platform, token, extra) {
    ...
    if (platform === "vk") {
      // 1. Пробуем users.get через api.vk.com
      let u = null;
      try {
        ...
        u = data && data.response && data.response[0];
      } catch { /* noop */ }
      // 2. Пробуем через api.vk.ru
      if (!u) {
        try {
          const data = await getJson(
            `https://api.vk.ru/method/users.get?fields=screen_name,domain&access_token=${encodeURIComponent(token)}&v=5.199`,
            { accept: "application/json" }
          );
          u = data && data.response && data.response[0];
        } catch { /* noop */ }
      }
      // 3. Пробуем через id.vk.ru/oauth2/user_info
      if (!u) {
        try {
          const ui = await postJson(
            `https://id.vk.ru/oauth2/user_info`,
            { client_id: APP_CONFIG.vk.clientId, access_token: token },
            { "content-type": "application/x-www-form-urlencoded" }
          );
          u = ui && (ui.user || ui);
        } catch { /* noop */ }
      }
      const rawId = (u && (u.id || u.user_id)) || (extra && extra.userId);
      if (!rawId) return null;
      const channel = String((u && (u.screen_name || u.domain)) || `id${rawId}`).toLowerCase();
      const name = (u && `${u.first_name || ""} ${u.last_name || ""}`.trim()) || channel;
      return { account: channel, channel, accountId: String(rawId), displayName: name };
    }
    return null;
  }
  ...
  /** Полное отключение: токен удаляется, бот выходит, подключение аккаунта сбрасывается. */
  revoke(platform) {
    const b = this.bots.get(platform);
    if (b) {
      this.teardown(b);
      b.joinAnnounced = false;
      b.followersBaselineReady = false;
      b.seenFollowers.clear();
      b.status = "offline";
    }
    this.saveBot(platform, { token: "", enabled: false, account: "", channel: "", accountId: "" });
    this.emitEvent(platform, "bot.leave", `Чат-бот ${platform === "twitch" ? "Twitch" : "VK"} отключён`);
    this.flushStatus();
    if (this.onToken) this.onToken({ platform, account: "", channel: "" });
  }
  ...
  teardown(b) {
    ...
    if (b.followersTimer) { clearTimeout(b.followersTimer); b.followersTimer = null; }
    if (b.ws) {
      const w = b.ws;
      b.ws = null;
      try { w.removeAllListeners(); w.terminate(); } catch { /* noop */ }
    }
    if (b.eventSubWs) {
      const ew = b.eventSubWs;
      b.eventSubWs = null;
      try { ew.removeAllListeners(); ew.terminate(); } catch { /* noop */ }
    }
    if (b.vkClient) {
      const vc = b.vkClient;
      b.vkClient = null;
      try { vc.disconnect && vc.disconnect(); vc.removeAllListeners && vc.removeAllListeners(); } catch { /* noop */ }
    }
  }
  ...
  connectTwitch(cfg) {
    ...
    ws.on("open", () => {
      ...
      send(`JOIN #${channel}`);
      b.ping = setInterval(() => send("PING :tmi.twitch.tv"), 240000);
    });
      ...
      for (const line of raw.toString().split("\r\n")) {
        ...
        if (msg.command === "001" || msg.command === "JOIN" || msg.command === "366") {
          if (b.status !== "online") {
            ...
            // Уведомление о подключении выдаём строго ОДИН РАЗ, без спама на каждый реконнект
            if (!b.joinAnnounced) {
              b.joinAnnounced = true;
              this.emitEvent("twitch", "bot.join", `Чат-бот Twitch подключён к #${channel}`);
            }
        ...
        /* ---- события USERNOTICE: подписки, подарки, рейды, баллы (автор строго YawaChatHub) ---- */
        if (msg.command === "USERNOTICE") {
          ...
          const map = {
            ...
            rewardgift: ["sys.reward", `🎁 ${who} подарил(а) награду ${recipient}`],
            viewermilestone: ["stream.viewers", `⭐ ${who} достиг(ла) отметки стрима: ${extra}`],
          };
          if (map[kind]) {
        ...
        if (msg.command === "PRIVMSG") {
          ...
          // 1) Награды за баллы канала с текстовым вводом пользователя:
          const customRewardId = msg.tags["custom-reward-id"];
          if (customRewardId) {
            const rewardKey = `irc:${author}:${text}`;
            if (!this.recentRewards.has(rewardKey)) {
              this.recentRewards.add(rewardKey);
              setTimeout(() => this.recentRewards.delete(rewardKey), 5000);
              this.emit("twitch", {
                author: "YawaChatHub",
          ...
          // 2) Bits (cheer) поддержка:
          const bits = msg.tags["bits"];
          if (bits) {
            this.emit("twitch", {
              author: "YawaChatHub",
              text: `💎 ${author} поддержал(а) канал (${bits} bits): ${text}`,
              badges: ["BOT"],
              kind: "sys.cheer",
            });
            continue;
    ...
    const run = async () => {
      ...
      try {
        ...
        if (Array.isArray(list)) {
          ...
          if (!b.followersBaselineReady) {
            // Первый опрос (baseline): только запоминаем существующих подписчиков, БЕЗ спама событиями!
            for (const f of list) {
              const name = f.user_name || f.user_login;
              if (name) b.seenFollowers.add(String(name).toLowerCase());
            }
            b.followersBaselineReady = true;
          } else {
            // Последующие опросы: эмитятся ТОЛЬКО новые подписчики
            for (const f of list) {
              const name = f.user_name || f.user_login;
              if (!name) continue;
      ...
      if (b.alive) b.followersTimer = setTimeout(run, 60000);
    };
    run();
  }
  ...
  async handleTwitchEventSubMessage(b, cfg, ws, raw) {
    ...
    if (subType === "channel.channel_points_custom_reward_redemption.add") {
      ...
      const dedupKey = `es:${who}:${reward}:${input}`;
      if (this.recentRewards.has(dedupKey)) return;
      this.recentRewards.add(dedupKey);
      setTimeout(() => this.recentRewards.delete(dedupKey), 5000);
      this.emit("twitch", {
        author: "YawaChatHub",
        text: `🎁 ${who} активировал(а) награду «${reward}»${input}`,
        badges: ["BOT"],
        kind: "sys.reward",
      });
    }
  ...
  /** Оформляет подписку на EventSub-топик через Helix. */
  async subscribeTwitchEventSub(cfg, sessionId) {
    try {
      const res = await this.helix("/eventsub/subscriptions", cfg.token, "POST", {
        type: "channel.channel_points_custom_reward_redemption.add",
        version: "1",
        condition: { broadcaster_user_id: cfg.accountId },
        transport: { method: "websocket", session_id: sessionId },
      });
      if (!res.ok && res.status !== 409) {
        console.warn("[eventsub] channel_points_custom_reward_redemption.add:", res.status, res.body?.slice(0, 200));
      }
    } catch (error) {
      console.warn("[eventsub] channel_points_custom_reward_redemption.add:", error && error.message);
    }
  }
  ...
  connectVk(cfg) {
    ...
    (async () => {
      try {
        ...
        if (!b.joinAnnounced) {
          b.joinAnnounced = true;
          this.emitEvent("vk", "bot.join", `Чат-бот VK подключён к #${channel}`);
        }
  ...
  watchVkFollowers(b, token, channel) {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (typeof n === "number") {
          if (!b.followersBaselineReady) {
            // Первый опрос (baseline): только запоминаем текущее число, БЕЗ спама!
            b.followersBaselineReady = true;
            b.followCount = n;
          } else if (n > b.followCount) {
            this.emit("vk", { author: "YawaChatHub", text: `🎉 Новый подписчик! Всего ${n} 👍`, badges: ["BOT"], kind: "sys.follow" });
            b.followCount = n;
          }
          this.flushStatus();
        }
