...
class BotManager {
  ...
  async handleOAuthToken(platform, token) {
    ...
    const info = await this.resolveAccount(platform, clean).catch(() => null);
    if (info) {
  ...
  /** Канал, которым владеет чат-бот, в виде записи для списка «Каналы». */
  managedChannel(platform) {
    const b = this.bots.get(platform);
    if (!b) return null;
    const cfg = this.botConfig(platform);
    const channel = b._main || cfg.channel || cfg.account;
    if (!channel) return null;
    return {
      id: `bot:${platform}:${String(channel).toLowerCase()}`,
      platform,
      channelId: channel,
    ...
    const run = async () => {
      ...
      try {
        ...
        if (Array.isArray(list) && list.length) {
          ...
          // Первый опрос — «посев»: существующие подписчики молча запоминаются,
          // чтобы после авторизации не приходил спам о старых подписках.
          const firstSeed = !b._followersSeeded;
          for (const f of list) {
          ...
          b._followersSeeded = true;
          b.lastEvent = `подписчиков: ${b.followCount}`;
          this.flushStatus();
        }
      } catch { /* noop */ }
      if (b.alive) b.retry = setTimeout(run, 120000);
    };
    run();
  }

  /* ============================================================
     Twitch EventSub (WebSocket) — награды за баллы, опросы,
     прогнозы, поезда поддержки, цели. Только события ПОСЛЕ
     авторизации: старые подписки Twitch удаляются при старте.
     ============================================================ */

  startEventSub(b, cfg) {
    if (b.es && b.es.alive) return;
    const token = String(cfg.token || "").replace(/^oauth:/, "");
    const clientId = this.twitchClientId();
    const broadcasterId = String(cfg.accountId || cfg.account || "");
    if (!token || !clientId || !broadcasterId) return;
    b.es = {
      token, clientId, broadcasterId, alive: true, ws: null,
      sessionId: null, subs: [], tries: 0,
    };
    this.esConnect(b);
  }
  ...
  stopEventSub(b) {
    if (b.es) {
      b.es.alive = false;
      if (b.es.retryTimer) { clearTimeout(b.es.retryTimer); }
      if (b.es.ws) { try { b.es.ws.terminate(); } catch { /* noop */ } }
      b.es = null;
    }
  }
  ...
  esConnect(b) {
    const es = b.es;
    if (!es || !es.alive || !b.alive) return;
    let ws;
    try {
      ws = new WebSocket("wss://eventsub.wss.twitch.tv/ws", tlsOptionsFor("wss://eventsub.wss.twitch.tv/ws"));
    } catch {
      return;
    }
    es.ws = ws;
    ws.on("message", (raw) => {
      ...
      if (type === "session_welcome") {
        es.sessionId = msg.payload && msg.payload.session && msg.payload.session.id;
        es.tries = 0;
        this.esEnsureSubscriptions(b);
      } else if (type === "notification") {
        this.esHandle(b, msg);
      } else if (type === "session_reconnect") {
        // Twitch сам предложит новый адрес; проще переподключиться заново —
        // старые подписки будут пересозданы после очистки.
    ...
    ws.on("close", () => {
      es.ws = null;
      es.sessionId = null;
      if (!es.alive || !b.alive) return;
      es.tries += 1;
      es.retryTimer = setTimeout(() => { if (es.alive && b.alive) this.esConnect(b); }, Math.min(3000 * es.tries, 20000));
    });
    ws.on("error", () => { try { ws.close(); } catch { /* noop */ } });
  }

  /** Создаём подписки после session_welcome (предварительно удалив старые). */
  async esEnsureSubscriptions(b) {
    const es = b.es;
    if (!es || !es.sessionId) return;
    try { await this.esCleanup(b); } catch { /* noop */ }
    const types = [
    ...
    for (const sub of types) {
      try {
        await this.helix("/eventsub/subscriptions", es.token, "POST", {
          type: sub.type,
          version: sub.version,
          condition: { broadcaster_user_id: es.broadcasterId },
          transport: { method: "websocket", session_id: es.sessionId },
        });
      } catch (error) {
        // Недостаточный scope или недоступный тип — не роняем остальные.
        console.warn("[eventsub]", sub.type, String(error && error.message || error).slice(0, 90));
      }
    }
  }

  /** Удаляем наши прошлые websocket-подписки (Twitch хранит их до 3 суток). */
  async esCleanup(b) {
    const es = b.es;
    if (!es) return;
    try {
      const data = await getJson(`${TWITCH_API}/eventsub/subscriptions?first=100`, {
        "Client-Id": es.clientId,
        Authorization: `Bearer ${es.token}`,
        accept: "application/json",
      });
      const rows = (data && data.data) || [];
      for (const row of rows) {
        if (!row || row.transport && row.transport.method !== "websocket") continue;
        if (row.condition && row.condition.broadcaster_user_id !== es.broadcasterId) continue;
        if (!row.id) continue;
        try {
          await this.helix(`/eventsub/subscriptions?id=${encodeURIComponent(row.id)}`, es.token, "DELETE");
        } catch { /* noop */ }
      }
    } catch { /* noop */ }
  }

  /** Разбор входящего события EventSub в сообщение ленты. */
  esHandle(b, msg) {
    const meta = msg.metadata || {};
    const type = meta.subscription_type || "";
    ...
    if (type === "channel.channel_points_custom_reward_redemption.add") {
      ...
      const input = String(event.user_input || "");
      const cost = reward.cost != null ? reward.cost : "";
      const text = `🎯 ${userName || "зритель"} активировал награду «${reward.title || title || "награда"}»`
        + (cost !== "" ? ` за ${cost} баллов` : "") + (input ? `: ${input}` : "");
      this.emit("twitch", { author: userName || "зритель", text, badges: ["BOT"], kind: "sys.reward" });
      return;
    }
    if (type === "channel.follow") {
      if (!userName) return;
      this.emit("twitch", { author: userName, text: `${userName} подписался на канал 🎉`, badges: ["BOT"], kind: "sys.follow" });
      return;
    }
    if (type === "channel.poll.begin") {
      this.emitEvent("twitch", "sys.poll", `Опрос начался: ${title || "без названия"}`);
      return;
    }
    if (type === "channel.poll.end") {
      this.emitEvent("twitch", "sys.poll", `Опрос завершён: ${title || "без названия"}`);
      return;
    }
    if (type === "channel.prediction.begin") {
      this.emitEvent("twitch", "sys.prediction", `Прогноз начался: ${title || "без названия"}`);
      return;
    }
    if (type === "channel.prediction.end") {
      this.emitEvent("twitch", "sys.prediction", `Прогноз завершён: ${title || "без названия"}`);
      return;
    }
    if (type === "channel.hype_train.begin") {
      const level = event.level != null ? ` Уровень ${event.level}` : "";
      this.emitEvent("twitch", "sys.hype", `🚂 Поезд поддержки начался!${level}`);
      return;
    }
    if (type === "channel.goal.begin") {
      this.emitEvent("twitch", "donation.goal", `🎯 Цель ${event.type || ""}: «${title || "без названия"}» — ${event.target_amount || "?"}`);
    }
  }
