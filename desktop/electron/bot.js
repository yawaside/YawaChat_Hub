...
class BotManager {
  ...
  emit(platform, { author, text, color, badges, kind, amount, currency, parts, login }) {
    if (!text) return;
    ...
    this.onChat({
      ...
      platform,
      author: finalAuthor,
      // login — идентификатор модерации (Twitch login / VK nick). Для бот-сообщений
      // модерация не нужна, поэтому login передаём только для реальных зрителей.
  ...
  teardown(b) {
    ...
    if (b.vkWatch) { clearTimeout(b.vkWatch); b.vkWatch = null; }
    ...
    ws.on("message", (raw) => {
      ...
      for (const line of raw.toString().split("\r\n")) {
        ...
        if (msg.command === "PRIVMSG") {
          ...
          this.emit("twitch", { author, login: user, text, color: msg.tags.color || "", badges, kind, parts: parts || undefined });
  ...
  connectVk(cfg) {
    ...
    // Варианты подключения (по приоритету):
    //  1. ПОД ВЛАДЕЛЬЦЕМ: auth: { accessToken } — токен владельца канала.
    //     Даёт приём сообщений И права модератора (отправка, баны).
    //  2. readonly на том же канале — гарантированный приём без прав.
    //  3. readonly на id<uid> — VK Video Live иногда использует числовой ID.
    // Если вариант «подключился», но за 40 секунд не прислал НИ ДАННЫХ
    // (сообщений/статуса стрима/информации о канале), пробуем следующий.
    const attempts = [];
    if (token) attempts.push({ mode: "token", ch: channel });
    attempts.push({ mode: "readonly", ch: channel });
    if (cfg.accountId) {
      const alt = `id${cfg.accountId}`;
      if (login(alt) !== login(channel)) attempts.push({ mode: "readonly", ch: alt });
    }
    ...
    const nextAttempt = async () => {
      ...
      if (attempt > attempts.length) {
        this.setStatus(b, "error", "VK: не удалось получить сообщения из чата канала");
        return;
      }
      const { mode, ch } = attempts[attempt - 1];
      let client = null;
      try {
        client = mode === "token"
          ? new VKPLMessageClient({ auth: { accessToken: token }, channels: [ch], debugLog: false, log: false })
          : new VKPLMessageClient({ auth: "readonly", channels: [ch], debugLog: false, log: false });
      } catch {
        nextAttempt();
        return;
      }
      b.vkClient = client;
      b.vkAuthMode = mode;
      b.vkData = false; // флаг «приходят данные» — гасит watchdog
      const markData = () => { b.vkData = true; };
      this._wireVkEvents(client, buildVkParts, ch, token, b, markData);
      try {
      ...
      b.joinedChannel = ch;
      this.setStatus(b, "online");
      this.emitEvent("vk", "bot.join", `VK: чат-бот подключён к ${ch} (${mode === "token" ? "владелец" : "readonly"})`);
      this.watchVkFollowers(b, token, ch);
      // Watchdog: 40 секунд тишины — пробуем следующий вариант.
      if (b.vkWatch) clearTimeout(b.vkWatch);
      b.vkWatch = setTimeout(() => {
        b.vkWatch = null;
        if (!b.alive || b.vkData) return;
        try { this.teardown(b); } catch { /* noop */ }
        b.alive = true;
        nextAttempt();
      }, 40000);
    };
  ...
  /** Навешивает обработчики сообщений/событий на VK-клиент.
   *  markData — вызывается при ЛЮБЫХ данных из чата (доказательство,
   *  что подключение рабочее и канал правильный). */
  _wireVkEvents(client, buildVkParts, channel, token, b, markData) {
    client.on("message", (ctx) => {
      ...
      // Для модерации VK (remove_user/unban) нужен ник или числовой ID зрителя —
      // ID надёжнее, ник на случай, если ID не пришёл.
      const vkLogin = (ctx && ctx.user && (ctx.user.id || ctx.user.nick)) || author;
      const badges = [];
    ...
    client.on("channel-info", () => { markData && markData(); });
    client.on("error", (e) => {
      if (b.alive) this.setStatus(b, "error", `VK клиент: ${String(e && e.message ? e.message : e).slice(0, 70)}`);
    });
    client.on("connect-error", (e) => {
      if (b.alive) this.setStatus(b, "error", `VK connect: ${String(e && e.message ? e.message : e).slice(0, 70)}`);
    });
  }
  ...
  /**
   * helix + проверка ответа. request НЕ бросает исключение при 401/403/404,
   * а возвращает { ok:false, status }. Раньше код полагался на catch, который
   * не срабатывал, и возвращал { ok:true } даже когда Twitch отклонил запрос.
   * Теперь отдаём реальную ошибку из тела ответа.
   */
  async helixOk(path, token, method = "GET", body = null) {
    const r = await this.helix(path, token, method, body).catch(() => null);
    if (!r) return { ok: false, error: "Нет ответа от Twitch API" };
    if (r.ok) return { ok: true };
    let detail = `HTTP ${r.status}`;
    try {
      const j = JSON.parse(r.body || "{}");
      detail = (j.message || j.error || detail).slice(0, 140);
    } catch { /* noop */ }
  ...
  async moderate(platform, { author, login: userLogin, action, seconds, text }) {
    ...
    if (platform === "twitch") {
      ...
      // Без гейта по статусу чата: модерация идёт через HTTP API Helix и
      // работает даже когда трансляция выключена (бот «не в чате»).
      ...
      if (action === "emoteonly" || action === "subscribers" || action === "clear") {
        if (action === "clear") {
          const r = await this.helixOk("/moderation/chat", token, "DELETE", null);
          if (!r.ok) return { ok: false, error: r.error };
          this.emitEvent("twitch", "mod.clear", "Чат очищен");
          return { ok: true };
        }
        const field = action === "emoteonly" ? "emote_mode" : "subscriber_mode";
        const r = await this.helixOk("/chat/settings", token, "PATCH", {
          broadcaster_id: broadcasterId, moderator_id: moderatorId, [field]: true,
        });
        if (!r.ok) return { ok: false, error: r.error };
        this.emitEvent("twitch", action === "emoteonly" ? "mod.emoteonly" : "mod.subscribers",
          action === "emoteonly" ? "Режим «только смайлы»" : "Режим «только сабы»");
        return { ok: true };
      }
      if (action === "slow") {
      ...
      if (action === "delete") {
        // API Helix не умеет удалить одно сообщение без его ID — честная ошибка.
        return { ok: false, error: "Twitch: удаление одного сообщения недоступно — используйте бан" };
      }

      /* Действия над пользователем: нужен user_id цели. */
      ...
      const path = `/moderation/bans?broadcaster_id=${encodeURIComponent(broadcasterId)}&moderator_id=${encodeURIComponent(moderatorId)}`;
      if (action === "ban") {
        const r = await this.helixOk(path, token, "POST", { data: { user_id: userId, reason: "YawaChatHub" } });
        if (!r.ok) return { ok: false, error: r.error };
        this.emitEvent("twitch", "mod.ban", `${target} забанен`);
      } else if (action === "unban") {
        // Для DELETE user_id передаётся в query-параметрах (не в теле запроса).
        const r = await this.helixOk(`${path}&user_id=${encodeURIComponent(userId)}`, token, "DELETE", null);
        if (!r.ok) return { ok: false, error: r.error };
        this.emitEvent("twitch", "mod.unban", `${target} разбанен`);
      } else if (action === "timeout") {
        const secs = Math.max(1, Number(seconds) || 300);
        const r = await this.helixOk(path, token, "POST", { data: { user_id: userId, duration: secs, reason: "YawaChatHub" } });
        if (!r.ok) return { ok: false, error: r.error };
    ...
    if (platform === "vk") {
      ...
      // Модерация VK идёт через HTTP API (live.vkvideo.ru), а не через
      // websocket-клиент, поэтому статус подключения чата не блокирует действие.
      const headers = { Authorization: `Bearer ${cfg.token}`, accept: "application/json" };
      const blog = encodeURIComponent(cfg.channel || cfg.account || "");
      if (!blog) return { ok: false, error: "Канал VK неизвестен — авторизуйтесь заново" };
      const callApi = async (endpoint, body) => {
        // postJson возвращает null при ошибке — используем request для проверки статуса.
        const r = await request(`https://api.live.vkvideo.ru/v1/blog/${blog}${endpoint}`, {
          method: "POST",
      ...
      try {
        if (action === "delete") {
          // VK Live не имеет API удаления одного сообщения — показываем подсказку.
          return { ok: false, error: "VK не поддерживает удаление сообщений — используйте бан" };
        }
        if (action === "unban") {
          let r = await callApi("/unban", { user: target });
          if (!r || !r.ok) r = await callApi("/remove_ban", { user: target });
          if (!r || !r.ok) {
            const reason = r ? `HTTP ${r.status}` : "нет ответа";
            return { ok: false, error: `VK не разбанил (${reason}). Проверьте права токена.` };
          }
        ...
        const r = await callApi("/remove_user", { user: target });
        if (!r || !r.ok) {
          const reason = r ? `HTTP ${r.status}` : "нет ответа";
          return { ok: false, error: `VK не забанил (${reason}). Проверьте права токена.` };
        }
