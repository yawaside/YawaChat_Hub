...
class BotManager {
  ...
  connectVk(cfg) {
     ...
     *   auth: { accessToken: <токен live.vkvideo.ru> }  — чтение И отправка
     *   auth: "readonly"                                  — только чтение
     *
     * РАНЬШЕ было наоборот: сначала всегда создавался readonly-клиент (он не
     * бросает исключение в конструкторе), поэтому токен НИКОГДА не использовался.
     * Отсюда не работали отправка сообщений и модерация, а чтение шло от
     * анонима. Теперь при наличии токена подключаемся с ним, readonly —
     * только запасной вариант. */
    let client = null;
    let authMode = "readonly";
    if (token) {
      try {
        client = new VKPLMessageClient({ auth: { accessToken: token }, channels: [channel], debugLog: false, log: false });
        authMode = "token";
      } catch (e) {
        console.warn("[vk] не удалось создать клиент с токеном, пробуем readonly:", e && e.message);
        client = null;
      }
    }
    if (!client) {
    ...
    b.vkClient = client;
    b.vkAuthMode = authMode;
    if (authMode === "readonly") {
      this.emitEvent("vk", "bot.join", "VK: подключено в режиме только чтение — отправка и модерация недоступны");
    }
    ...
    // Ошибки клиента (в т.ч. отклонённый токен) — без этого подключение
    // молча «висело» в состоянии online, хотя чат не работал.
    try {
      client.on("error", (e) => {
    ...
    (async () => {
      try {
        ...
        this.setStatus(b, "online");
        if (firstJoin) {
          this.emitEvent("vk", "bot.join", `VK: чат-бот подключён к ${channel} (${authMode === "token" ? "полный доступ" : "только чтение"})`);
          this.watchVkFollowers(b, token, channel);
        }
      } catch (e) {
        if (!b.alive) return;
        // Токен отклонён — пробуем переподключиться readonly, чтобы чат хотя бы читался.
        const msg = String(e && e.message || e);
        console.warn("[vk] подключение с токеном не удалось:", msg);
        if (authMode === "token") {
          try {
            const ro = new VKPLMessageClient({ auth: "readonly", channels: [channel], debugLog: false, log: false });
            b.vkClient = ro;
            b.vkAuthMode = "readonly";
            this.teardown({ ...b, ws: null, vkClient: null, ping: null, retry: null });
            ro.on("message", () => {});
            await ro.connect();
            b.joinedChannel = channel;
            this.setStatus(b, "online");
            this.emitEvent("vk", "bot.join", `VK: подключено только чтение — токен отклонён (${msg.slice(0, 50)})`);
            return;
          } catch { /* noop */ }
        }
        this.setStatus(b, "error", msg.slice(0, 90));
      }
    })();
  }
