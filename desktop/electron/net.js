 ...
 *
 * Проблема: у части пользователей стоит антивирус или корпоративный прокси с
 * TLS-инспекцией. Его корневой сертификат добавлен в хранилище Windows, поэтому
 * Chromium ему доверяет, а Node — нет: соединения падали с
 * "unable to verify the first certificate".
 *
 * Раньше это лечили строкой NODE_TLS_REJECT_UNAUTHORIZED = "0", которая отключала
 * проверку сертификатов во всём процессе. Это небезопасно и конфликтует с
 * политиками безопасности. Теперь поступаем аккуратно:
 *   1) по умолчанию проверяем сертификат как обычно;
 *   2) если цепочку не удалось проверить именно из-за локальной инспекции
 *      (частые коды ниже) — пропускаем ТОЛЬКО известные чат-хосты;
 *   3) любой другой хост и любая другая ошибка по-прежнему отклоняются.
 */
...
/**
 * Агент для ws/https: доверяет системному хранилищу, а при ошибке инспекции
 * пропускает только чат-хосты из списка выше.
 */
const chatAgent = new https.Agent({
  keepAlive: true,
  // Держим не больше пары «спящих» соединений: экономит память в простое.
  maxCachedSessions: 16,
  checkServerIdentity(host, cert) {
    const err = tls.checkServerIdentity(host, cert);
    if (err && isTrustedHost(host)) {
      debug(`[tls] имя сертификата не совпало для ${host} — пропускаем как известный чат-хост`);
      return undefined;
    }
    return err;
  },
...
/**
 * Опции для `new WebSocket(url, opts)`: тот же агент и мягкая проверка
 * только для доверенных доменов.
 */
function tlsOptionsFor(url) {
  let host = "";
  try { host = new URL(String(url).replace(/^ws/, "http")).hostname; } catch { /* noop */ }
  if (!isTrustedHost(host)) return { agent: chatAgent };
  return {
    agent: chatAgent,
    // rejectUnauthorized НЕ выключаем: сначала обычная проверка, и только
    // характерная ошибка инспекции считается допустимой.
