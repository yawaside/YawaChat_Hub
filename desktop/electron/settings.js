...
function loadSettings(baseDir) {
  ...
  const migrateToV4 = Number(data.settingsSchemaVersion || 0) < 4;
  ...
  // v4: структура чат-бота появилась — не трогаем существующие настройки,
  // просто гарантируем, что все поля бота на месте.
  if (migrateToV4 && !data.bot) {
    settings.bot = JSON.parse(JSON.stringify(DEFAULTS.bot));
  }
  if (!settings.token) {
