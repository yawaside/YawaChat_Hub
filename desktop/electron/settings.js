...
/** Копия настроек с зашифрованными секретами — то, что пишется в settings.json. */
...
function saveSettings(settings, baseDir) {
  try {
    // Пишем копию с зашифрованными секретами; в памяти остаётся открытое значение.
    secure.setKeyPath(path.join(baseDir || getBaseDir(), "settings.key"));
    const toWrite = encryptSecretsCopy(settings);
    fs.writeFileSync(getSettingsPath(baseDir), JSON.stringify(toWrite, null, 2), "utf8");
  } catch (e) {
