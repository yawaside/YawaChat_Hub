...
const APP_CONFIG = {
  ...
  vk: {
    ...
    /* VK ID (OAuth 2.1) принимает только собственные права. Старое значение
       "offline" из VK API 5.x здесь недопустимо: authorize возвращал ошибку
       либо выдавал токен без данных пользователя, и приложение не могло
       определить аккаунт — вход «проходил», но в интерфейсе не появлялся. */
