...
/** Проверка, есть ли рендерер в сборке. Если нет — пишем причину в error.log,
 *  чтобы «окно не появляется» можно было диагностировать без догадок. */
function checkRenderer() {
  try {
    if (!require("fs").existsSync(RENDERER)) {
      bootLog(`НЕТ рендерера: ${RENDERER}. Соберите интерфейс (npm run build:renderer) и скопируйте dist → desktop/renderer-dist.`);
      return false;
    }
    return true;
  } catch (e) {
    bootLog(`Ошибка проверки рендерера: ${e && e.message ? e.message : e}`);
    return false;
  }
}
...
function createMainWindow() {
  ...
  /* ВСЯ диагностика окна вешается ДО loadFile: иначе событие успевает пройти
     до подписки и в логе выглядит как «рендерер не загрузился», хотя он загрузился. */
  mainWin.webContents.on("did-start-loading", () => bootLog(`renderer: started loading`));
  mainWin.webContents.on("did-finish-load", () => {
    bootLog(`renderer: loaded OK +${Date.now() - bootStart}ms`);
  });
  mainWin.webContents.on("did-fail-load", (_e, code, desc, url) => {
    bootLog(`renderer: ОШИБКА загрузки ${code} ${desc} ${url}`);
  });
  mainWin.webContents.on("preload-error", (_e, p, err) => {
    bootLog(`renderer: ОШИБКА preload ${p}: ${err && err.message ? err.message : err}`);
  });
  mainWin.webContents.on("render-process-gone", (_e, details) => {
    bootLog(`renderer: процесс упал ${JSON.stringify(details)}`);
  });
  // Ошибки JavaScript в окне — самая частая причина «белого окна».
  mainWin.webContents.on("console-message", (_e, level, message, line, source) => {
    if (level >= 3) bootLog(`renderer console: ${message} (${source}:${line})`);
  });
  mainWin.once("ready-to-show", () => {
    bootLog(`renderer: ready-to-show +${Date.now() - bootStart}ms`);
    if (!settings.startHidden) mainWin.show();
    else bootLog(`окно НЕ показано: в settings.json startHidden=true`);
  });

  mainWin.loadFile(RENDERER, { hash: "/app" });
  // Диагностика и страховка: если рендерер не пришёл (did-fail-load / таймаут),
  // показываем окно с текстом ошибки вместо «невидимого» приложения.
  mainWin.webContents.on("did-fail-load", (_e, code, desc) => {
    bootLog(`рендерер не загрузился (${code} ${desc}). Проверьте desktop/renderer-dist/index.html`);
  });
  /* Последняя линия обороны: окно ОБЯЗАНО появиться. Настройка startHidden
     здесь игнорируется сознательно — иначе неверно сохранённый флаг навсегда
     оставляет приложение «запущенным, но невидимым». */
  const showFallback = setTimeout(() => {
    ...
    if (!checkRenderer()) {
      mainWin.loadURL(
        ...
        encodeURIComponent("<body style='background:#0b0d16;color:#e8eaf2;font-family:sans-serif;padding:30px'>" +
          "<h2>YawaChatHub не может запуститься</h2>" +
          "<p>Не найден интерфейс (renderer-dist/index.html).</p>" +
          "<p>Соберите его командой <code>npm run build:renderer</code> и скопируйте <code>dist</code> в <code>desktop/renderer-dist</code>.</p>" +
          "</body>")
      );
    }
    mainWin.show();
    bootLog(`окно показано страховкой +${Date.now() - bootStart}ms (startHidden=${settings.startHidden}, видимость была false)`);
  }, 6000);
...
app.whenReady().then(async () => {
  ...
  registerHotkeys();
  createMainWindow();
  bootLog(`boot window created +${Date.now() - bootStart}ms | renderer ${require("fs").existsSync(RENDERER) ? "OK" : "ОТСУТСТВУЕТ: " + RENDERER}`);
  createTray();
  ...
  /* Обработчики OBS-виджета для интерфейса.
     В некоторых прошлых сборках их не было в main.js (остались только в preload),
     из-за чего рендерер не получал фактический порт/токен, и ссылка собиралась
     заведомо мёртвой (http://127.0.0.1:0/widget?token=) — «как для демо».
     Регистрируем один раз; виджет-сервер уже запущен выше. */
  if (widgetServer && widgetServer.ready && !widgetServer.__ipcDone) {
    widgetServer.__ipcDone = true;
    const baseUrl = widgetServer.port
      ? `http://127.0.0.1:${widgetServer.port}/widget?token=${encodeURIComponent(settings.token)}`
      : "";

    ipcMain.handle("widget:url", () => baseUrl);
    ipcMain.handle("widget:info", () => ({
      port: widgetServer.port || 0,
      token: String(settings.token || ""),
      url: baseUrl,
      ready: true,
      clients: typeof widgetServer.clientCount === "function" ? widgetServer.clientCount() : 0,
    }));

    // Тестовое сообщение в виджет (кнопка «Протестировать» в настройках).
    ipcMain.on("widget:test", (_e, msg) => {
      if (!msg) return;
      try {
        if (typeof widgetServer.broadcast === "function") widgetServer.broadcast(msg);
      } catch { /* noop */ }
    });

    // Изменение оформления применяется в уже открытом OBS сразу.
    ipcMain.on("widget:config", (_e, payload) => {
      if (!payload) return;
      try {
        if (typeof widgetServer.sendConfig === "function") widgetServer.sendConfig(payload);
      } catch { /* noop */ }
    });
  }
  ...
  if (mainWin && !mainWin.isDestroyed()) {
    // Подписка ставится ПОСЛЕ await, поэтому загрузка могла уже завершиться —
    // проверяем состояние явно, иначе коннекторы ждут событие, которое не придёт.
    if (!mainWin.webContents.isLoading()) {
      bootLog(`renderer already loaded — старт коннекторов сразу`);
      setTimeout(startConnectors, 150);
    } else {
      mainWin.webContents.once("did-finish-load", () => setTimeout(startConnectors, 150));
      setTimeout(startConnectors, 3000); // подстраховка
    }
  } else {
    startConnectors();
  }
