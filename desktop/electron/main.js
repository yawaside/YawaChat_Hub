...
const { authorizeInApp } = require("./oauth");
...
/* ============================================================
   ...
   ============================================================ */
ipcMain.handle("oauth:start", async (_e, platform) => {
  ...
  try {
    const token = await authorizeInApp({
      platform,
    ...
    if (message !== "Авторизация отменена") console.warn("[oauth]", platform, message);
