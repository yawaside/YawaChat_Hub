...
function showMainWindow() {
  if (!mainWin || mainWin.isDestroyed()) return;
  if (mainWin.isMinimized()) mainWin.restore();
  // Выдвигаем окно поверх остальных при возврате из браузера, затем снимаем фиксацию
 ...
 * Сайт передаёт сюда данные площадки (POST или GET) + state; мы сверяем state
 * и прокидываем итоговый токен в бота. Пользователю токен нигде не показываем.
 *
...
h1{font-size:18px;margin:0 0 8px}p{color:#9aa2c0;font-size:13px;line-height:1.5}</style></head><body><div class="card"><h1>${title}</h1>${body}</div></body></html>`;
  ...
  async function exchangeVkCode({ code, deviceId, state, verifier }) {
    ...
    // 1) Пробуем id.vk.ru/oauth2/auth
    let res = await request("https://id.vk.ru/oauth2/auth", {
      method: "POST",
    ...
    // 2) Запасной endpoint: id.vk.com/oauth2/auth
    if (!res.ok || !data || !data.access_token) {
      res = await request("https://id.vk.com/oauth2/auth", {
        method: "POST",
        headers: { "content-type": "application/x-www-form-urlencoded" },
        body,
        timeout: 15000,
  ...
  const server = http.createServer((req, res) => {
    ...
    if (!m) { res.writeHead(404, CORS); res.end("YawaChatHub OAuth relay"); return; }
    const platform = m[1];
    ...
    req.on("end", async () => {
      ...
      try {
        if (platform === "twitch") {
          token = String((payload && payload.token) || u.searchParams.get("access_token") || u.searchParams.get("token") || "").trim();
          ...
          const exchangeResult = await exchangeVkCode({ code, deviceId, state, verifier: expected.verifier });
          token = exchangeResult.token;
          extra = { userId: exchangeResult.userId };
        }
      } catch (error) {
        send(400, "Авторизация не удалась", `<p>${String(error && error.message ? error.message : error).slice(0, 200)}</p>`);
        return;
      }

      delete pendingOAuth[platform];
      // Прокидываем токен в бота, возвращаем фокус в окно приложения
      bot.handleOAuthToken(platform, token, extra).catch(() => {});
      showMainWindow();
...
ipcMain.on("channels:remove", (_e, c) => {
  ...
  // Если отключили канал (не через авторизацию) — сбрасываем подключение аккаунта бота!
...
ipcMain.handle("oauth:start", (_e, platform) => {
  ...
  if (platform === "twitch") {
    ...
    pendingOAuth.vk = { state, verifier };
...
app.whenReady().then(async () => {
  ...
  bot = new BotManager({
    ...
    // Авторизация сразу подключает канал через обычный ConnectorManager —
    // он появляется в «Каналы» с онлайном/зрителями. Дубли текста сообщений
    // гасятся в connectors.js (_isAuthManaged): текст идёт только от бота.
    onAuthorizedChannel: (platform, channelId) => {
      if (connectors && (platform === "twitch" || platform === "vk")) connectors.add(platform, channelId);
    },
  });
