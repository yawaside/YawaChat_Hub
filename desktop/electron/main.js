 ...
 *  3. После выдачи разрешений браузер возвращается на сайт с токеном в адресе.
 *  4. Сайт (OAuthCallback.tsx) читает токен, НЕ показывает его и передаёт в
 *     локальный сервер приложения (127.0.0.1:17563) вместе со state.
 *  5. Локальный сервер сверяет state (защита от CSRF), прокидывает токен в бота
 *     и сам возвращает фокус в окно приложения.
 * state привязан к площадке и случаю — чужой сайт не сможет подложить токен.
 */
const pendingOAuth = {}; // platform -> { state }
...
/**
 * Обмен authorization code на access_token для VK ID.
 * VK ID требует client_secret на этом шаге (поэтому секрет зашит в сборку —
 * это штатная схема desktop-приложений VK ID).
 */
...
/**
 * Локальный приёмник токена. Привязан к 127.0.0.1 — наружу не выходит.
 * Сайт передаёт сюда access_token (POST) + state; мы сверяем state и прокидываем
 * токен в бота. Токен нигде не показывается пользователю.
 */
...
h1{font-size:18px;margin:0 0 8px}p{color:#9aa2c0;font-size:13px;line-height:1.5}</style></head><body><div class="card"><h1>${title}</h1>${body}</div></body></html>`;
  ...
  const server = http.createServer((req, res) => {
    ...
    if (!m) { res.writeHead(404, { "access-control-allow-origin": "*" }); res.end("YawaChatHub OAuth relay"); return; }
    const platform = m[1];

    // Токен приходит в POST-теле (JSON) или в query. state сверяем обязательно.
    let body = "";
    ...
    req.on("end", () => {
      ...
      if (payload && typeof payload.token === "string") token = payload.token.trim();
      if (payload && typeof payload.code === "string") code = payload.code.trim();
      if (payload && typeof payload.state === "string") state = payload.state;
      token = token || u.searchParams.get("access_token") || u.searchParams.get("token") || "";
      code = code || u.searchParams.get("code") || "";

      const expected = pendingOAuth[platform];
      if (expected && state && expected.state !== state) {
        send(403, "Авторизация не подтверждена", "<p>Проверка state не прошла. Откройте приложение и повторите вход.</p>");
        return;
      }
      ...
      // VK ID отдаёт authorization code — обмениваем его на access_token.
      if (platform === "vk" && code && !token) {
        exchangeVkCode(code)
          .then((accessToken) => {
            if (!accessToken) {
              send(400, "VK: не удалось обменять код", "<p>Проверьте защищённый ключ в app-config.js и повторите вход.</p>");
              return;
            }
            bot.handleOAuthToken("vk", accessToken).catch(() => {});
            showMainWindow();
            send(200, "VK подключён", "<p>Готово! Фокус вернулся в YawaChatHub. Можно закрыть эту вкладку.</p>");
          })
          .catch(() => send(400, "VK: ошибка обмена кода", "<p>Повторите вход в приложении.</p>"));
        return;
      }
      if (!token) {
        send(400, "Токен не получен", "<p>В данных нет access_token. Повторите вход в приложении.</p>");
        return;
      }
      // Прокидываем токен в бота и возвращаем фокус в приложение.
      bot.handleOAuthToken(platform, token).catch(() => {});
      showMainWindow();
      send(200, platform === "twitch" ? "Twitch подключён" : "VK подключён",
        "<p>Готово! Фокус вернулся в YawaChatHub. Можно закрыть эту вкладку.</p>");
    });
  });
...
/**
 * Единый список каналов для интерфейса:
 *  - DonationAlerts (если настроен);
 *  - каналы, которыми владеет чат-бот (Twitch/VK после авторизации);
 *  - обычные коннекторы.
 * Канал, которым уже владеет бот, НЕ дублируется коннектором (иначе сообщения
 * шли бы дважды: анонимный коннектор + авторизованный бот).
 */
function botManagedChannels() {
  const out = [];
  if (!bot) return out;
  if (bot.donationChannel) {
    const da = bot.donationChannel();
    if (da) out.push(da);
  }
  if (bot.managedChannel) {
    for (const p of ["twitch", "vk"]) {
      const ch = bot.managedChannel(p);
      if (ch) out.push(ch);
    }
  }
  return out;
...
function mergeChannelList(list) {
  const managed = botManagedChannels();
  const rest = (list || []).filter((c) => {
    if (c.platform === "donation") return false;
    const dup = managed.find((m) => m.platform === c.platform &&
      String(m.channelId).toLowerCase() === String(c.channelId).toLowerCase());
    return !dup;
  });
  return [...managed, ...rest];
}

/** Удаляет из коннекторов канал, которым теперь владеет бот (дедупликация). */
function syncBotChannelsToConnectors() {
  if (!connectors) return;
  for (const p of ["twitch", "vk"]) {
    const managed = bot && bot.managedChannel ? bot.managedChannel(p) : null;
    if (!managed) continue;
    connectors.remove(p, managed.channelId);
  }
}
...
ipcMain.on("channels:add", (_e, c) => {
  ...
  // Если каналом уже владеет чат-бот — не создаём второй коннектор (дубли чата).
  const managed = bot && bot.managedChannel ? bot.managedChannel(c.platform) : null;
  if (managed && String(managed.channelId).toLowerCase() === String(c.channelId).toLowerCase()) {
    emit("sp:channels", mergeChannelList(connectors ? connectors.list() : []));
    return;
  }
...
ipcMain.on("channels:remove", (_e, c) => {
  ...
  // Снятие канала бота (авторизованного Twitch/VK) отключает бота от площадки.
  const managed = bot && bot.managedChannel ? bot.managedChannel(c.platform) : null;
  if (managed && String(managed.channelId).toLowerCase() === String(c.channelId).toLowerCase()) {
    bot?.revoke(c.platform);
    return;
  }
  connectors?.remove(c.platform, c.channelId);
});
ipcMain.handle("channels:list", () => mergeChannelList(connectors ? connectors.list() : []));
...
ipcMain.handle("oauth:start", (_e, platform) => {
  ...
  if (platform === "twitch") {
    ...
    // VK ID (id.vk.com): authorization code flow. Старый oauth.vk.com с этим
    // приложением даёт Security Error — у него другой механизм.
    url = `${cfg.authorizeUrl}?client_id=${encodeURIComponent(cfg.clientId)}`
      + `&redirect_uri=${redirect}`
      + `&response_type=code`
      + `&scope=${encodeURIComponent(cfg.scopes.join(","))}`
      + `&v=${encodeURIComponent(cfg.apiVersion || "5.199")}`
...
app.whenReady().then(async () => {
  ...
  connectors = new ConnectorManager({
    ...
    onStatus: (list) => emit("sp:channels", mergeChannelList(list)),
  });
  ...
  /* ============================================================
     ...
     ============================================================ */
  const emitChannels = () => emit("sp:channels", mergeChannelList(connectors ? connectors.list() : []));
  ...
  bot = new BotManager({
    ...
    onBotStatus: (list) => {
      ...
      // Каналы бота (Twitch/VK) видны в разделе «Каналы» и меняют статус сразу.
      try { emit("sp:channels", mergeChannelList(connectors ? connectors.list() : [])); } catch { /* noop */ }
    },
    onChannels: emitChannels,
    onToken: (info) => {
      // Авторизация прошла: канал теперь ведёт бот — убираем дубль-коннектор.
      try { syncBotChannelsToConnectors(); } catch { /* noop */ }
      emitChannels();
