...
h1{font-size:18px;margin:0 0 8px}p{color:#9aa2c0;font-size:13px;line-height:1.5}</style></head><body><div class="card"><h1>${title}</h1>${body}</div></body></html>`;
  ...
  async function exchangeVkCode({ code, deviceId, state, verifier }) {
    ...
    // Пробуем оба домена: id.vk.com (международный) и id.vk.ru (РФ)
    let res = await request("https://id.vk.com/oauth2/auth", { method: "POST", headers, body, timeout: 12000 });
    if (!res.ok) {
      res = await request("https://id.vk.ru/oauth2/auth", { method: "POST", headers, body, timeout: 12000 });
    }
    let data = null;
  ...
  const server = http.createServer((req, res) => {
    ...
    req.on("end", async () => {
      ...
      try {
        ...
        send(400, "Авторизация не удалась", String(error && error.message ? error.message : error).slice(0, 200));
      }
    });
  });
...
ipcMain.on("channels:remove", (_e, c) => {
  ...
  // Если отключен канал Twitch/VK — сбрасываем подключение аккаунта бота к этому каналу (не авторизация)
  if (bot && (c.platform === "twitch" || c.platform === "vk")) {
    bot.onChannelRemoved(c.platform, c.channelId);
  }
});
