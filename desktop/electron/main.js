...
// Аварийный режим: на некоторых видеодрайверах (старые Intel HD, виртуалки, RDP)
// окно Chromium остаётся ПУСТЫМ при включённом GPU-ускорении. Запуск с ключом
// --no-gpu (или YAWACHAT_NO_GPU=1) отключает ускорение и чинит белое окно.
if (process.argv.includes("--no-gpu") || process.env.YAWACHAT_NO_GPU === "1") {
  app.disableHardwareAcceleration();
  bootLog("аппаратное ускорение отключено (--no-gpu)");
}
...
function createMainWindow() {
  ...
  // Диагностика загрузки рендерера. Регистрируем ДО loadFile, иначе быстрые
  // события (single-file HTML грузится за миллисекунды) проходят мимо.
  mainWin.webContents.on("dom-ready", () => {
    bootLog(`renderer dom-ready +${Date.now() - bootStart}ms`);
    // Показываем окно как только есть DOM — не ждём ready-to-show,
    // который при некоторых драйверах GPU может не прийти вовсе.
    if (!settings.startHidden && mainWin && !mainWin.isDestroyed() && !mainWin.isVisible()) {
      mainWin.show();
      bootLog(`window shown on dom-ready +${Date.now() - bootStart}ms`);
    }
  });
  mainWin.webContents.on("render-process-gone", (_e, details) => {
    bootLog(`РЕНДЕРЕР УПАЛ: ${details && details.reason} (exitCode ${details && details.exitCode})`);
  });
  mainWin.webContents.on("preload-error", (_e, preloadPath, error) => {
    bootLog(`ОШИБКА preload.js (${preloadPath}): ${error && error.message ? error.message : error}`);
  });
  // Ошибки JavaScript внутри интерфейса — самая частая причина «белого окна».
  mainWin.webContents.on("console-message", (_e, level, message, line, sourceId) => {
    if (level >= 2) bootLog(`renderer console [${level}] ${message} (${sourceId}:${line})`);
  });

  mainWin.loadFile(RENDERER, { hash: "/app" });
  bootLog(`loadFile вызван, startHidden=${settings.startHidden === true}`);
  mainWin.once("ready-to-show", () => {
    bootLog(`ready-to-show +${Date.now() - bootStart}ms`);
  ...
  const showFallback = setTimeout(() => {
    if (!checkRenderer()) {
      if (mainWin && !mainWin.isDestroyed() && !settings.startHidden) {
        mainWin.loadURL(
    ...
    } else if (mainWin && !mainWin.isDestroyed() && !mainWin.isVisible() && !settings.startHidden) {
      mainWin.show();
...
ipcMain.on("channels:remove", (_e, c) => {
  ...
  // Отключение канала, который был добавлен АВТОРИЗАЦИЕЙ (точное совпадение),
  // — это полный выход: токен удаляется, бот отключается.
  // Прочие каналы — просто сброс подключения, авторизация не трогается.
...
app.whenReady().then(async () => {
  ...
  if (mainWin && !mainWin.isDestroyed()) {
    // ВАЖНО: между createMainWindow() и этим местом стоит `await startWidgetServer`,
    // поэтому рендерер (single-file HTML) часто успевает загрузиться РАНЬШЕ, чем мы
    // подпишемся. Тогда once("did-finish-load") не срабатывал никогда, и коннекторы
    // стартовали только по запасному таймеру 3 с. Проверяем факт загрузки явно.
    if (!mainWin.webContents.isLoading()) {
      bootLog(`renderer уже загружен к моменту подписки +${Date.now() - bootStart}ms`);
