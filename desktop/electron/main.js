...
const { getBaseDir, loadSettings, saveSettings, flushSettings, initSafeStorage } = require("./settings");
// secure.js загружается лениво внутри settings.js. Объявляем переменную null,
// чтобы при запуске на старом кэше portable (где этот код ещё был) не падал
// ReferenceError — переменная просто будет falsy и код пропустится.
const secure = null;
...
app.whenReady().then(async () => {
  ...
  // secure.js загружается лениво внутри settings.js → initSafeStorage.
  try {
