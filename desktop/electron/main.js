...
/** Ожидание OAuth: { platform, state, at }. State — одноразовый маркер:
 *  защита от того, чтобы токен из чужой/старой вкладки попал в приложение. */
...
/* ---------------- локальный мост токена (127.0.0.1) ---------------- */

/**
 * Страница сайта yawachathub.netlify.app после входа площадки пересылает
 * токен сюда. Мост:
 *  - слушает ТОЛЬКО 127.0.0.1 (извне недоступен);
 *  - сверяет одноразовый state из oauth:start (защита от подмены токена);
 *  - отдаёт токен боту и возвращает фокус в окно приложения.
 */
...
.card{max-width:460px;padding:28px;border-radius:18px;border:1px solid #2a2f45;background:#141828;text-align:center}
h1{font-size:18px;margin:0 0 8px}p{color:#9aa2c0;font-size:13px;line-height:1.5}</style></head><body><div class="card"><h1>${title}</h1>${body}</div></body></html>`;
  ...
  const respond = (res, code, title, body) => {
    if (res.writableEnded) return;
    res.writeHead(code, { "content-type": "text/html; charset=utf-8", ...CORS });
    res.end(page(title, body));
  };

  const isStateValid = (platform, state) => {
    const pending = pendingOAuth;
    const fresh = pending && pending.platform === platform && Date.now() - pending.at < 600000;
    if (!fresh) return false;
    return state ? pending.state === state : true;
  };
  ...
  const server = http.createServer((req, res) => {
    ...
    if (!m) {
      ...
      res.end("YawaChatHub OAuth bridge");
      return;
    }
    const platform = m[1];

    const finish = (token, state) => {
      if (!isStateValid(platform, state)) {
        pendingOAuth = null;
        return respond(res, 400, "Некорректный запрос",
          "<p>Состояние авторизации (state) не совпадает. Начните вход заново из YawaChatHub.</p>");
      }
      if (!token) {
        pendingOAuth = null;
        return respond(res, 400, "Токен не получен",
          "<p>В ссылке нет access_token. Начните авторизацию заново.</p>");
      }
      if (!bot) {
        // Приложение ещё не готово: state НЕ снимаем — повторный запрос
        // со страницы сайта пройдёт, как только бот поднимется.
        return respond(res, 503, "Приложение ещё запускается",
          "<p>Подождите пару секунд — токен будет принят автоматически.</p>");
      }
      // Токен принят: state снимается только здесь, сохраняем токен,
      // определяем канал, фокус — в окно приложения.
      pendingOAuth = null;
      bot.handleOAuthToken(platform, token).finally(() => showMainWindow());
      respond(res, 200, platform === "twitch" ? "Twitch подключён" : "VK подключён",
        "<p>Токен передан в YawaChatHub, окно приложения уже открыто. Вкладку можно закрыть.</p>");
    };
    ...
    if (req.method === "POST") {
      ...
      req.on("end", () => {
        ...
        try {
          ...
          token = token || String(p.access_token || "");
          state = state || String(p.state || "");
        } catch { /* тело не JSON — используем query */ }
        finish(token, state);
      });
      return;
    }
    finish(u.searchParams.get("access_token") || "", u.searchParams.get("state") || "");
  });

  server.on("error", (e) => console.warn("[oauth] мост не запущен:", e && e.message));
  server.listen(port, host, () => console.log(`[oauth] мост на http://${host}:${port}`));
}
...
/* ============================================================
   Авторизация по площадкам — системный браузер + redirect на сайт
   ============================================================
   1. Приложение открывает системный браузер с URL авторизации площадки.
   2. Redirect URI у обеих площадок один: https://yawachathub.netlify.app/
      (зарегистрирован при создании приложений).
   3. После выдачи прав площадка возвращает пользователя на сайт с
      #access_token=…&state=….
   4. Страница сайта (src/components/OAuthCallback.tsx) читает токен и state
      и пересылает их локальному мосту http://127.0.0.1:17563/auth/<площадка>.
   5. Мост проверяет state, сохраняет токен, определяет канал и возвращает
      фокус в окно приложения — пользователь снова в приложении.
   ============================================================ */
ipcMain.handle("oauth:start", (_e, platform) => {
  ...
  if (!cfg.clientId) {
    return { ok: false, error: "Не задан Client ID в desktop/electron/app-config.js" };
  }
  const state = `${platform}.${crypto.randomBytes(16).toString("hex")}`;
  pendingOAuth = { platform, state, at: Date.now() };
  const redirect = encodeURIComponent(APP_CONFIG.redirectUrl);
  const scopes = (cfg.scopes || []).join(platform === "twitch" ? " " : ",");
  const scopeParam = scopes ? `&scope=${encodeURIComponent(scopes)}` : "";
  const url = platform === "twitch"
    ? `https://id.twitch.tv/oauth2/authorize?client_id=${encodeURIComponent(cfg.clientId)}`
      + `&redirect_uri=${redirect}&response_type=token${scopeParam}&state=${encodeURIComponent(state)}`
    : `https://oauth.vk.com/authorize?client_id=${encodeURIComponent(cfg.clientId)}`
      + `&redirect_uri=${redirect}&response_type=token&display=page&v=5.199${scopeParam}&state=${encodeURIComponent(state)}`;
...
app.whenReady().then(() => {
  // Окно создаётся ПЕРВЫМ: интерфейс появляется сразу, а остальное поднимается
  // параллельно — иначе старт «залипал» на несколько секунд.
  registerHotkeys();
  createMainWindow();
  createTray();
  // Мост токена обязан быть готов ДО того, как пользователь начнёт вход:
  // страница сайта шлёт токен на http://127.0.0.1:17563 сразу после редиректа.
  startOAuthServer();

  // Прогрев Edge TTS отложен: не тянет за собой запуск главного окна.
  setTimeout(() => { try { warmupEdge(); } catch { /* noop */ } }, 1500);

  // Виджет-сервер поднимается асинхронно (без await): порты и OBS не должны
  // блокировать создание коннекторов и чат-бота. widgetServer — module-level.
  startWidgetServer({
    port: settings.port,
  ...
  }).then((s) => { widgetServer = s; }).catch(() => { widgetServer = null; });
  ...
  connectors = new ConnectorManager({
    ...
    onStatus: (list) => emit("sp:channels", mergeDonationChannel(list)),
  });
  ...
  /* ============================================================
     ...
     ============================================================ */
  const emitChannels = () => emit("sp:channels", mergeDonationChannel(connectors ? connectors.list() : []));
