...
/**
 * Локальный мост авторизации (127.0.0.1:17563).
 *
 * Площадки возвращают пользователя на сайт https://yawachathub.netlify.app/.
 * Страница сайта читает access_token из адресной строки и передаёт его сюда
 * запросом /auth/:platform?access_token=… — так токен попадает в приложение,
 * хотя redirect-адрес у обеих площадок один и тот же.
 */
...
h1{font-size:18px;margin:0 0 8px}p{color:#9aa2c0;font-size:13px;line-height:1.5}</style></head><body><div class="card"><h1>${title}</h1>${body}</div></body></html>`;
  ...
  const server = http.createServer((req, res) => {
    ...
    if (error) {
      ...
      res.end(page("Авторизация не удалась", `<p>${String(error).slice(0, 200)}</p>`));
      return;
    }
    if (!token) {
      res.writeHead(400, { "content-type": "text/html; charset=utf-8", ...CORS });
      res.end(page("Токен не получен", "<p>В ссылке нет access_token. Проверьте настройки приложения площадки.</p>"));
      return;
    }
    // Токен получен: сохраняем, узнаём аккаунт/канал и подключаем бота.
    if (bot) {
      bot.handleOAuthToken(platform, token).catch(() => {});
      res.writeHead(200, { "content-type": "text/html; charset=utf-8", ...CORS });
      res.end(page(
        platform === "twitch" ? "Twitch подключён" : "VK подключён",
        "<p>Токен передан в YawaChatHub. Окно приложения можно закрыть из браузера.</p>"
      ));
    } else {
      res.writeHead(503, { "content-type": "text/html; charset=utf-8", ...CORS });
      res.end(page("Приложение не запущено", "<p>Откройте YawaChatHub и повторите авторизацию.</p>"));
    }
  });
...
ipcMain.handle("channels:list", () => {
  const list = connectors ? connectors.list() : [];
  return bot?.donationChannel ? [...list.filter((c) => c.platform !== "donation"), bot.donationChannel()] : list;
});
...
/* ============================================================
   Авторизация по площадкам — браузер по умолчанию
   ============================================================ */
...
ipcMain.handle("oauth:start", (_e, platform) => {
  ...
  let url = "";
  if (platform === "twitch") {
    if (!APP_CONFIG.twitch.clientId) {
      return { ok: false, error: "Client ID Twitch не задан в desktop/electron/app-config.js" };
    }
    url = `https://id.twitch.tv/oauth2/authorize?client_id=${encodeURIComponent(APP_CONFIG.twitch.clientId)}`
      + `&redirect_uri=${redirect}&response_type=token&scope=${encodeURIComponent(APP_CONFIG.twitch.scopes.join(" "))}`;
  } else if (platform === "vk") {
    if (!APP_CONFIG.vk.clientId) {
      return { ok: false, error: "ID приложения VK не задан в desktop/electron/app-config.js" };
    }
    url = `https://oauth.vk.com/authorize?client_id=${encodeURIComponent(APP_CONFIG.vk.clientId)}`
      + `&redirect_uri=${redirect}&response_type=token&scope=${encodeURIComponent(APP_CONFIG.vk.scopes.join(","))}&display=page`;
  } else {
    return { ok: false, error: "Неизвестная площадка" };
  }
  shell.openExternal(url).catch(() => {});
  return { ok: true, url };
});
...
app.whenReady().then(async () => {
  ...
  connectors = new ConnectorManager({
    ...
    onStatus: (list) => {
      // если есть канал DonationAlerts — добавляем его в список площадок
      if (bot && bot.donationChannel) {
        emit("sp:channels", [...list.filter((c) => c.platform !== "donation"), bot.donationChannel()]);
      } else {
        emit("sp:channels", list);
      }
    },
  });
  ...
  const emitChannels = () => {
    const list = connectors ? connectors.list() : [];
    // DonationAlerts показывается как обычный канал/площадка с суммой донатов.
    if (bot && bot.donationChannel) {
      const da = bot.donationChannel();
      const merged = list.filter((c) => c.platform !== "donation");
      merged.push(da);
