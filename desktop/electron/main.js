...
function openAuthWindow(platform, url) {
  ...
  /* Сетевой перехват — самая надёжная точка. Страница сайта-редиректа даже не
     начинает грузиться: мы забираем code/token прямо из адреса. Именно поэтому
     CORS и блокировка запросов к 127.0.0.1 (Private Network Access) больше
     не влияют на вход — сайт в обмене не участвует. */
  try {
    const base = String(APP_CONFIG.redirectUrl || "").split("?")[0];
    if (base) {
      wc.session.webRequest.onBeforeRequest({ urls: [`${base}*`] }, (details, callback) => {
        if (inspect(details.url)) {
