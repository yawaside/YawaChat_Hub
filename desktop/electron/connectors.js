...
class ConnectorManager {
  ...
  async _tiktok(entry) {
    ...
    // tiktok-live-connector ≥2.x — чистый ESM ("type": "module"), поэтому грузим
    // через динамический import(), как и VK-клиент. Обычный require() здесь
    // всегда падает с ERR_REQUIRE_ESM — именно он давал ошибку «библиотека чата
    // недоступна в этой сборке», хотя библиотека установлена.
    let Conn = null;
    let loadError = "";
    try {
      const mod = await import("tiktok-live-connector");
      Conn =
        mod.TikTokLiveConnection ||           // v2.x — актуальное API
        mod.WebcastPushConnection ||          // v1.x — на случай отката версии
        (mod.default && (mod.default.TikTokLiveConnection || mod.default.WebcastPushConnection)) ||
        null;
    } catch (e) {
      loadError = String((e && e.message) || e);
      // Запасной путь для старых CJS-сборок библиотеки (<2.x).
      try {
        // eslint-disable-next-line global-require
        const mod = require("tiktok-live-connector");
        Conn = mod && (mod.TikTokLiveConnection || mod.WebcastPushConnection || mod.default);
      } catch { /* ниже уйдём в офлайн с понятным текстом */ }
    }
    if (typeof Conn !== "function") {
      const reason = loadError ? ` (${loadError.slice(0, 60)})` : "";
      this._offline(entry, 120, `TikTok: библиотека чата не загрузилась${reason}`);
      return;
    }
