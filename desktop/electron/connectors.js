...
const { getJson, postJson, request, setDebug, tlsOptionsFor, chatAgent } = require("./net");
...
class ConnectorManager {
  ...
  async _tiktok(entry) {
    ...
    try {
      ...
      const rid = liveRoom?.roomId || userObj?.roomId || room?.data?.roomId;
      const users = liveRoom?.liveRoomStats?.userCount ?? liveRoom?.userCount;

      if (typeof users === "number") initialViewers = users;
      if (rid && String(rid) !== "0") discoveredRoomId = String(rid);

      // status 2 = LIVE — используется как подсказка, основная логика
      // определения эфира остаётся за событиями самой библиотеки.
    } catch { /* noop */ }
