...
const { getJson, postJson, request, setDebug, tlsOptionsFor } = require("./net");
...
class ConnectorManager {
  ...
  /**
   * true, если этот канал уже читает авторизованный чат-бот (settings.bot.*).
   * Анонимное подключение для такого канала продолжает работать (даёт онлайн
   * и число зрителей — карточка видна в «Каналы»), но текст сообщений не
   * эмитится — иначе одно и то же сообщение приходило бы дважды: от бота
   * и от обычного (анонимного) коннектора.
   */
  _isAuthManaged(platform, channelId) {
    if (platform !== "twitch" && platform !== "vk") return false;
    const cfg = this.settings.bot && this.settings.bot[platform];
    if (!cfg || !cfg.token || !cfg.channel) return false;
    const norm = (s) => String(s || "").trim().toLowerCase().replace(/^[@#]/, "");
    return norm(cfg.channel) === norm(channelId);
  }
  ...
  _wsCheck(url, { hello, okWhen, headers } = {}) {
    return new Promise((resolve) => {
      ...
      try {
        ws = new WebSocket(url, { ...tlsOptionsFor(url), ...(headers ? { headers } : {}) });
      } catch (e) {
        clearTimeout(t);
  ...
  _twitch(entry) {
    ...
    let ws;
    try {
      ws = new WebSocket(TWITCH_WS, tlsOptionsFor(TWITCH_WS));
    } catch {
    ...
    ws.on("message", (raw) => {
      for (const line of raw.toString().split("\r\n")) {
        ...
        // Канал уже читает авторизованный бот — не дублируем текст сообщения.
        if (this._isAuthManaged("twitch", name)) continue;
  ...
  _kickPusher(entry, slug, chatroomId) {
    const tryKey = (idx) => {
      ...
      let ws;
      try {
        ws = new WebSocket(kickPusherUrl(KICK_PUSHER[idx]), { ...WS_HEADERS, ...tlsOptionsFor(kickPusherUrl(KICK_PUSHER[idx])) });
      } catch {
        tryKey(idx + 1);
        return;
      }
  ...
  /* ---------------- TikTok Live ---------------- */
  async _getTikTokConnector() {
    if (this._cachedTikTokConn) return this._cachedTikTokConn;
    ...
    // В packaged-приложении ПЕРВЫМ берём физический файл из app.asar.unpacked.
    // Dynamic import ESM из виртуального app.asar ненадёжен, особенно когда
    // импортируемые proto/SDK пакеты были подняты npm в корневой node_modules.
    const candidates = [];
    if (process.resourcesPath) {
      candidates.push(
        path.join(process.resourcesPath, "app.asar.unpacked", "node_modules", "tiktok-live-connector", "dist", "index.js"),
        path.join(process.resourcesPath, "app.asar.unpacked", "node_modules", "tiktok-live-connector", "dist", "legacy.js")
      );
    }
    // Dev/неупакованная сборка.
    candidates.push(
    ...
    for (const filePath of candidates) {
      ...
      try {
        ...
        const Conn = resolveClass(await import(fileUrl));
        if (Conn) {
          this._cachedTikTokConn = Conn;
          return Conn;
        }
      } catch (error) {
        errors.push(`${filePath}: ${error?.code || error?.message || error}`);
      }
    }

    // Bare import — запасной путь для dev и новых Electron, где ESM из asar
    // поддерживается корректно.
    try {
      const Conn = resolveClass(await import("tiktok-live-connector"));
      if (Conn) {
        this._cachedTikTokConn = Conn;
        return Conn;
      }
    } catch (error) {
      errors.push(`import: ${error?.code || error?.message || error}`);
    }

    // Совместимость с прежними CommonJS версиями библиотеки.
    try {
      const Conn = resolveClass(require("tiktok-live-connector"));
      if (Conn) {
        this._cachedTikTokConn = Conn;
        return Conn;
      }
    } catch (error) {
      errors.push(`require: ${error?.code || error?.message || error}`);
    }

    console.error("[tiktok] не удалось загрузить коннектор:", errors.join(" | "));
    this._tiktokLoadError = errors.join(" | ");
    return null;
  }
  ...
  async _tiktok(entry) {
    ...
    const Conn = await this._getTikTokConnector();
    if (!Conn) {
      this._offline(entry, 60,
        `TikTok: библиотека не загрузилась${this._tiktokLoadError ? ` — ${this._tiktokLoadError.slice(0, 90)}` : ""}`
      );
      return;
    }

    // Поиск активной комнаты и начального онлайна через Chromium net-стек
    ...
    try {
      ...
      // Сама библиотека считает data.user.roomId авторитетным. liveRoom.roomId
      // может сохраняться от прошлого эфира, поэтому используем его только
      // если API не сообщил status=4 (offline).
      ...
      if (typeof users === "number") initialViewers = users;
      if (status !== 4 && rid && String(rid) !== "0") discoveredRoomId = String(rid);
    } catch { /* noop */ }
    ...
    try {
      client = new Conn(username, {
        ...
        // Библиотека сама ходит за подписанным WebSocket через got и ws.
        // На машинах с антивирусом/TLS-инспекцией эти соединения без мягкого
        // агента не устанавливаются: комната находится (онлайн виден), но
        // push-сокет молчит и чат не приходит. Прокидываем тот же агент,
        // что и для остальных площадок.
        webClientOptions: {
        ...
        wsClientOptions: {
          // ws: тот же мягкий агент, что и у собственных коннекторов
          ...tlsOptionsFor("wss://webcast.tiktok.com/webcast/im/push/v2/"),
    ...
    // Диагностика: websocket-рукопожатие прошло. Если «онлайн виден, но чат
    // молчит», этот системный лог покажет, дошли ли мы до push-сокета.
    client.on("websocketConnected", () => {
      this.emitSys("tiktok", `TikTok: чат-сокет подключён / @${username}`);
    });
    ...
    client
      ...
      .catch((e) => {
        ...
        // Composite-ошибки (не нашли комнату ни одним способом) — это «не в эфире»,
        ...
        this._retry(entry, isOffline ? 60000 : 20000, {
          note: isOffline ? `TikTok: @${username} сейчас не в эфире` : `TikTok: @${username} — ${msg}`,
        });
      });

    // без error-обработчика EventEmitter бросает процесс с любой сетевой ошибкой коннектора.
    // Но причину пишем в диагностику, иначе «нет сообщений» невозможно отличить
    // от сетевого сбоя push-сокета.
    client.on("error", (payload) => {
      const info = payload?.info || payload?.exception?.message || payload?.message || "";
      if (info && entry.alive) {
        this.emitSys("tiktok", `TikTok сокет: ${String(info).slice(0, 70)}`);
      }
    });
    ...
    // В v2 библиотека всегда сначала выдаёт decodedData(method, decoded),
    // и только затем пытается сопоставить method со стандартным событием chat.
    // На некоторых сборках сопоставление WebcastChatMessage не срабатывает,
    // хотя protobuf уже успешно декодирован. Поэтому принимаем ОБА пути.
    // WeakSet исключает дубль: decoded.data и аргумент chat — тот же объект.
    ...
    const emitTikTokChat = (payload) => {
      ...
      // Совместимость фигур сообщения:
      // v2/v3 — user{nickname, uniqueId}; legacy — плоские поля.
      const user = d.user || d.userInfo || d.message?.user || {};
      const author =
        user.nickname || user.uniqueId || user.unique_id ||
        d.nickname || d.uniqueId || d.unique_id || "зритель";

      let parts;
      const emotes = Array.isArray(d.emotes) ? [...d.emotes] : [];
      if (emotes.length && typeof comment === "string") {
        parts = [];
        emotes.sort((x, y) => (x?.placeInComment ?? 0) - (y?.placeInComment ?? 0));
        let lastIdx = 0;
        for (const emote of emotes) {
          const at = emote?.placeInComment ?? 0;
          const url = emote?.emote?.image?.imageUrl ?? emote?.emoteImageUrl ??
            emote?.image?.imageUrl ?? null;
          if (at > lastIdx) parts.push({ type: "text", value: comment.slice(lastIdx, at) });
          if (url) parts.push({
            type: "emote",
            value: emote?.name || emote?.emoteName || comment[at] || "emote",
            url,
          });
          lastIdx = at + 1;
        }
        if (lastIdx < comment.length) parts.push({ type: "text", value: comment.slice(lastIdx) });
        if (!parts.some((part) => part.type === "emote")) parts = undefined;
      }

      this.emitChat("tiktok", { author, text: String(comment), parts });
    };

    // Прямой путь из protobuf — не зависит от внутренней таблицы WebcastEventMap.
    ...
    // Стандартный публичный API библиотеки — оставляем для legacy и будущих версий.
  ...
  async _vk(entry) {
    ...
    client.on("message", (ctx) => {
      if (!entry.alive) return;
      // Канал уже читает авторизованный бот — не дублируем текст сообщения
      // (анонимное подключение продолжает работать для онлайна/зрителей).
      if (this._isAuthManaged("vk", channelId)) return;
