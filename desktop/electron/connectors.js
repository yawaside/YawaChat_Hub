...
class ConnectorManager {
  ...
  _teardown(entry) {
    ...
    if (entry.viewersTimer) { clearTimeout(entry.viewersTimer); entry.viewersTimer = null; }
    // Сторож тишины TikTok: снимаем вместе с остальными таймерами канала.
  ...
  async _getTikTokConnector() {
    ...
    // 1. Динамический import (для ESM пакета в Node/Electron)
    try {
      const mod = await import("tiktok-live-connector");
      const Conn = mod?.TikTokLiveConnection || mod?.default?.TikTokLiveConnection || mod?.WebcastPushConnection;
      if (Conn) {
    ...
    // 2. Стандартный require (если Node 22+ с require-module)
    try {
    ...
    // 3. Загрузка из распакованного asar (app.asar.unpacked) в собранном приложении
    if (process.resourcesPath) {
      const unpackedCandidates = [
        ...
        path.join(process.resourcesPath, "app.asar.unpacked", "desktop", "node_modules", "tiktok-live-connector", "dist", "index.js"),
    ...
    // 4. Поиск пути относительно директории desktop
  ...
  /**
   * Сведения о комнате TikTok из официального endpoint.
   * Возвращает { roomId, viewers, isLive }; при любой ошибке — пустой результат,
   * тогда подключение пойдёт штатным путём библиотеки.
   */
  async _lookupTikTokRoom(username) {
    try {
      const room = await getJson(
        `https://www.tiktok.com/api-live/user/room?aid=1988&sourceType=54&uniqueId=${encodeURIComponent(username)}`,
        { referer: `https://www.tiktok.com/@${username}`, accept: "application/json" }
      ...
      const user = room?.data?.user;
      // У пользователя roomId точнее: liveRoom иногда хранит сведения прошлого эфира.
      const rid = user?.roomId || liveRoom?.roomId || room?.data?.roomId;
      const viewers = liveRoom?.liveRoomStats?.userCount ?? liveRoom?.userCount;
      const roomId = rid && String(rid) !== "0" ? String(rid) : null;
      // В ответе TikTok: status 2 — идёт эфир, 4 — офлайн.
      const isLive = user?.status === 2 || liveRoom?.status === 2;
      return {
        roomId,
        viewers: typeof viewers === "number" ? viewers : null,
        isLive: Boolean(isLive),
      };
    } catch {
      return { roomId: null, viewers: null, isLive: false };
    }
  }
  ...
  async _tiktok(entry) {
    ...
    if (!Conn) {
      this._offline(entry, 60, "TikTok: библиотека чата недоступна в этой сборке");
      return;
    }

    // Запасные сведения о комнате: берём их через Chromium-стек, у которого есть
    // куки и настоящий User-Agent, — TikTok отдаёт ему данные охотнее.
    const { roomId: discoveredRoomId, viewers: initialViewers, isLive: channelIsLive } =
      await this._lookupTikTokRoom(username);

    let client;
    try {
      client = new Conn(username, {
        processInitialData: true,
        // Проверка комнаты обязательна: библиотека сама бросит UserOfflineError,
        // если канал не в эфире, и попутно наполнит куки, нужные вебсокету.
    ...
    /* Подписки ставим ДО connect().
       При processInitialData библиотека разбирает первый пакет сообщений прямо
       внутри connect(), поэтому подписка после вызова теряла эти сообщения. */
    let received = 0;
    ...
    client.on("chat", (d) => {
      ...
      let parts;
      const text = typeof d?.comment === "string" ? d.comment : String(d?.content || "");
      const emotes = Array.isArray(d?.emotes) ? [...d.emotes] : [];
      if (emotes.length && text) {
        parts = [];
        ...
        for (const e of emotes) {
          ...
          if (at > lastIdx) parts.push({ type: "text", value: text.slice(lastIdx, at) });
          if (url) {
            parts.push({ type: "emote", value: e?.name || text[at] || "emote", url });
          }
          lastIdx = at + 1;
        }
    ...
    /* Подключение.
       ...
       Сначала идём штатным путём: библиотека ищет комнату сама, у неё для этого
       несколько источников подряд. Свой roomId (из официального endpoint через
       Chromium-стек) держим ЗАПАСНЫМ вариантом и подставляем, только если
       штатный поиск не удался.

       Почему не подставлять сразу: при неточном идентификаторе вебсокет
       открывается «в пустоту» — соединение считается успешным, канал числится
       онлайн, а сообщения не приходят. */
    const isOffline = (e) =>
      e?.name === "UserOfflineError" ||
      e?.constructor?.name === "UserOfflineError" ||
      /offline|not online|isn't online|not live/i.test(String(e?.message || ""));

    Promise.resolve()
      .then(() => client.connect())
      .catch((e) => {
        if (!entry.alive || isOffline(e) || !discoveredRoomId) throw e;
        return client.connect(discoveredRoomId);
      })
      .then((state) => {
        if (!entry.alive) return;
        ...
        /* Сторож тишины: соединение установлено, но данных из комнаты нет.
           Такое бывает при неверно определённой комнате — канал молча висел бы
           «в эфире». Ждём и переподключаемся, вместо тишины без объяснений. */
        if (entry.silenceTimer) clearTimeout(entry.silenceTimer);
        entry.silenceTimer = setTimeout(() => {
          entry.silenceTimer = null;
          if (!entry.alive || entry.closing || received > 0) return;
          this._retry(entry, 3000, {
            note: `TikTok: @${username} — комната не отвечает, переподключаюсь`,
          });
        }, 90000);
        // Сторож не должен удерживать процесс при выходе из приложения.
        entry.silenceTimer.unref?.();
      })
      .catch((e) => {
        if (!entry.alive) return;
        ...
        this._retry(entry, offline ? 60000 : 20000, {
          note: offline
            ? `TikTok: @${username} сейчас не в эфире`
            : channelIsLive
            ? `TikTok: @${username} в эфире, но подключиться не вышло — ${msg}`
            : `TikTok: @${username} — ${msg}`,
        });
      });
