...
check('Build icons referenced by electron-builder exist and are valid', () => {
  ...
  assert.deepEqual([...Array(count)].map((_, i) => {
    ...
    // Кадры со сжатием PNG — формат Vista+, как в prepare-icon.mjs.
...
/**
 * Каждый локальный require в desktop/electron должен указывать на файл, который
 * РЕАЛЬНО закоммичен (а значит, попадёт в checkout CI и в asar).
 *
 * Зачем: релизы 1.2.0–1.2.2 падали при запуске с «Cannot find module './secure'» —
 * модуль существовал только на машине разработчика и не был закоммичен, поэтому
 * упакованное приложение крашилось ещё до появления окна.
 */
check('Every local require in desktop/electron resolves to a committed file', () => {
  const dir = path.join(root, 'desktop/electron');
  let tracked = null;
  try {
    tracked = new Set(
      execFileSync('git', ['ls-files', 'desktop/electron'], { cwd: root, encoding: 'utf8' })
        .split('\n').map((s) => s.trim()).filter(Boolean)
    );
  } catch { /* вне git-репозитория — проверяем только наличие файлов на диске */ }

  for (const name of fs.readdirSync(dir).filter((f) => f.endsWith('.js'))) {
    const source = fs.readFileSync(path.join(dir, name), 'utf8');
    for (const match of source.matchAll(/require\(\s*['"](\.[^'"]+)['"]\s*\)/g)) {
      const target = path.join(dir, match[1]);
      const resolved = [target, `${target}.js`, path.join(target, 'index.js')].find((p) => fs.existsSync(p));
      assert.ok(resolved, `${name}: require('${match[1]}') не находит файл`);
      if (tracked) {
        const rel = path.relative(root, resolved).split(path.sep).join('/');
        assert.ok(tracked.has(rel), `${rel} не закоммичен — в сборке модуль будет отсутствовать`);
      }
    }
  }
});
...
/**
 * Шифрование настроек не должно зависеть от отдельного файла: иначе при его
 * потере приложение молча переходит на открытое хранение токенов.
 */
check('Settings encryption is built into settings.js, not an optional module', () => {
  const source = file('desktop/electron/settings.js');
  assert.ok(source.includes('aes-256-gcm'), 'settings.js содержит реализацию шифрования');
  assert.ok(source.includes('scryptSync'), 'ключ выводится через scrypt');
  assert.ok(!/require\(\s*['"]\.\/secure['"]\s*\)/.test(source), 'settings.js не зависит от ./secure');
  assert.ok(!/require\(\s*['"]\.\/secure['"]\s*\)/.test(file('desktop/electron/main.js')), 'main.js не зависит от ./secure');
});

/**
 * Портативная сборка распаковывается в %TEMP%\<unpackDirName> при КАЖДОМ запуске.
 * Имя обязано содержать версию: с общим именем разные версии делят один каталог,
 * и занятая папка приводит к запуску старой сборки.
 */
check('Portable unpack directory is version-specific', () => {
  const pkg = JSON.parse(file('desktop/package.json'));
  const { unpackDirName } = pkg.build.portable;
  assert.ok(unpackDirName.includes(pkg.version), `unpackDirName (${unpackDirName}) должен содержать версию ${pkg.version}`);
});
