 ...
 * РЕАЛЬНО есть в репозитории (а значит, попадёт в checkout CI и в asar).
 *
 * Зачем: релиз 1.1.8 падал при запуске с «Cannot find module './secure'» —
 * модуль существовал только на машине разработчика и не был закоммичен,
 * поэтому упакованное приложение крашилось ещё до появления окна.
 */
