...
check('Chat platforms stay reachable on machines with TLS-inspecting antivirus', () => {
  const main = read('desktop/electron/main.js');
  const net = read('desktop/electron/net.js');

  // У пользователей с антивирусом/прокси, инспектирующим TLS, «сырой» Node не
  // доверяет его корню. Чат-библиотеки (tiktok-live-connector и др.) открывают
  // собственные соединения, в которые агент не пробросить, поэтому без
  // глобальной настройки у таких машин не работает НИ ОДНА площадка.
  // Приложение читает только публичные чаты, а переменная ограничена процессом.
  const enabled = main.split('\n').some((line) => {
    const code = line.trim();
    if (code.startsWith('//') || code.startsWith('*')) return false;
    return /process\.env\.NODE_TLS_REJECT_UNAUTHORIZED\s*=\s*"0"/.test(code);
  });
  assert.ok(enabled, 'совместимость с TLS-инспекцией обязана быть включена');

  // Для соединений, которыми управляем сами, мягкость ограничена чат-хостами:
  // остальной трафик идёт через строгий агент.
  assert.ok(net.includes('rejectUnauthorized: false'), 'у чат-агента мягкая проверка');
  assert.ok(net.includes('secureAgent'), 'для прочих хостов есть строгий агент');
  assert.ok(net.includes('isTrustedHost'), 'поблажка ограничена списком чат-хостов');
  assert.ok(net.includes('installCertificateHandler'), 'ошибки сертификатов разбираются и для net-запросов');
...
/* ─────────── TikTok: загрузка ESM-библиотеки и поток сообщений ─────────── */

check('TikTok connector loads as ESM and resolves the connection class', () => {
  const connectors = read('desktop/electron/connectors.js');
  // В Electron 33 (Node 20) require() ESM-пакета падает с ERR_REQUIRE_ESM —
  // именно из-за этого была «библиотека недоступна». Должен быть динамический import.
  assert.ok(connectors.includes('_getTikTokConnector'), 'есть асинхронный загрузчик');
  assert.ok(/await import\(["']tiktok-live-connector["']\)/.test(connectors),
    'загрузчик использует динамический import()');
  // Ищем запасные пути к app.asar.unpacked, иначе в собранном exe import не сработает.
  assert.ok(connectors.includes('app.asar.unpacked'), 'есть путь к распакованному asar');
  assert.ok(connectors.includes('"dist"') && connectors.includes('"index.js"'),
    'импорт указывает на dist/index.js');
});

check('TikTok library is unpacked from asar so dynamic import works in the exe', () => {
  const pkg = JSON.parse(read('desktop/package.json'));
  const unpack = pkg.build.asarUnpack || [];
  const required = [
    'tiktok-live-connector', 'tiktok-live-api-sdk', 'tiktok-live-proto',
    '@bufbuild/protobuf', 'axios', 'form-data', 'follow-redirects',
    'proxy-from-env', 'ws', 'vklive-message-client',
  ];
  for (const dependency of required) {
    assert.ok(unpack.some(p => p.includes(dependency)),
      `${dependency} должен быть в asarUnpack`);
  }
});

check('TikTok passes TLS agents into the library own HTTP and WebSocket clients', () => {
  const connectors = read('desktop/electron/connectors.js');
  // Симптом «онлайн виден, а сообщений нет»: комната находится нашим net-стеком,
  // но внутренний got/ws библиотеки не проходит через TLS-инспекцию и push-сокет молчит.
  assert.ok(connectors.includes('webClientOptions'), 'got-клиент получает опции');
  assert.ok(connectors.includes('wsClientOptions'), 'ws-клиент получает опции');
  assert.ok(connectors.includes('tlsOptionsFor("wss://webcast.tiktok.com'),
    'push-сокет использует мягкий TLS-агент');
  assert.ok(connectors.includes('rejectUnauthorized: false'),
    'got-клиент не падает на TLS-инспекции');
});

check('TikTok connects straight to a discovered room id when available', () => {
  const connectors = read('desktop/electron/connectors.js');
  // Передаём найденный roomId в connect(), минуя внутренний HTML-скрейпинг,
  // который TikTok часто блокирует капчей.
  assert.ok(/\.connect\(discoveredRoomId \|\| undefined\)/.test(connectors),
    'roomId передаётся в connect()');
  assert.ok(connectors.includes('api-live/user/room'),
    'roomId ищется через официальный api-live endpoint');
  assert.ok(connectors.includes('fetchRoomInfoOnConnect: !discoveredRoomId'),
    'повторный запрос инфо не делается, если комната уже найдена');
});

check('TikTok chat events flow through to the shared chat emitter', () => {
  // Программно воспроизводим путь обработчика: данные события chat должны
  // превращаться в сообщение с автором/текстом/частями эмодзи.
  const connectors = read('desktop/electron/connectors.js');
  assert.ok(/client\.on\(["']decodedData["']/.test(connectors),
    'прямо принимает декодированный WebcastChatMessage');
  assert.ok(connectors.includes('WebcastChatMessage'), 'фильтрует правильный protobuf method');
  assert.ok(/client\.on\(["']chat["']/.test(connectors), 'сохраняет публичное событие chat как fallback');
  assert.ok(connectors.includes('seenChatObjects'), 'двойной путь не создаёт дубли');
  assert.ok(connectors.includes('user.nickname || user.uniqueId'), 'v2/v3: автор из user');
  assert.ok(connectors.includes('d.nickname || d.uniqueId'), 'legacy: плоские поля автора');
  assert.ok(connectors.includes('this.emitChat("tiktok"'), 'сообщение уходит в emitChat');
});

check('net request does not crash on a timer declared in temporal dead zone', () => {
  // Раньше при ошибке до объявления таймера finish() обращался к TDZ-переменной
  // и ронял процесс вместо аккуратного ответа об ошибке.
  const net = read('desktop/electron/net.js');
  assert.ok(/let timer = null;/.test(net), 'таймер объявлен до создания запроса');
  assert.ok(/if \(timer\) clearTimeout\(timer\)/.test(net), 'clearTimeout защищён проверкой');
});
