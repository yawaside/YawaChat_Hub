...
check('TikTok library loads as ESM, not via bare require()', () => {
  const src = read('desktop/electron/connectors.js');
  // tiktok-live-connector ≥2.x — чистый ESM ("type": "module"): синхронный
  // require() всегда падает с ERR_REQUIRE_ESM, что и давало ошибку
  // «библиотека чата недоступна в этой сборке» при установленной библиотеке.
  assert.ok(src.includes('await import("tiktok-live-connector")'),
    'TikTok обязан грузиться динамическим import()');
  // require() допустим лишь как запасной путь для старых CJS-сборок.
  const requires = [...src.matchAll(/require\("tiktok-live-connector"\)/g)];
  assert.ok(requires.length <= 1, 'единственный require — только fallback');
  // Поддерживаем оба поколения API: v2 (TikTokLiveConnection) и v1 (WebcastPushConnection).
  assert.ok(src.includes('TikTokLiveConnection'), 'поддерживается API v2');
  assert.ok(src.includes('WebcastPushConnection'), 'поддерживается API v1');
  // Диагностика вместо глухого «недоступна»: причина видна в ленте.
  assert.ok(src.includes('библиотека чата не загрузилась'), 'ошибка загрузки показывает причину');
});
