...
check('TikTok library is unpacked from asar so dynamic import works in the exe', () => {
  ...
  assert.ok(unpack.some(p => p.includes('tiktok-live-connector')),
    'tiktok-live-connector должен быть в asarUnpack');
  assert.ok(unpack.some(p => p.includes('vklive-message-client')),
    'vklive-message-client тоже распаковывается (ESM)');
});
...
check('TikTok chat events flow through to the shared chat emitter', () => {
  ...
  const connectors = read('desktop/electron/connectors.js');
  assert.ok(/client\.on\(["']chat["']/.test(connectors), 'подписан на событие chat');
  assert.ok(/d\?\.user\?\.nickname/.test(connectors), 'v2: автор из d.user.nickname');
  assert.ok(/d\?\.nickname \|\| d\?\.uniqueId/.test(connectors), 'v1: плоские поля автора');
