...
export function writeVersion(version) {
  ...
  pkg.version = version;
  /* Портативная сборка распаковывается во временную папку с этим именем.
     Имя обязано меняться вместе с версией: при общем имени разные версии
     делят один каталог в %TEMP%, и если он занят (запущенная копия, антивирус),
     NSIS не может его очистить и запускает СТАРУЮ сборку. */
  if (pkg.build && pkg.build.portable) {
    pkg.build.portable.unpackDirName = `YawaChatHub-${version}`;
  }
